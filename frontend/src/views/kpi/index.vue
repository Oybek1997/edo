    ...
    <v-card class="ma-1 pa-1">
      <v-card-title>
        ...
        <v-toolbar
          ...
          dense
          class="mx-1"
          v-if="!$store.getters.checkPermission('kpi-manager')"
        >
          <v-spacer></v-spacer>
          <v-btn
            v-if="
          ...
          ></v-autocomplete>
          <v-btn
            v-if="saveComission || (saveOwner && kpi_user)"
            color="success"
            outlined
            @click="save()"
            class="ml-1"
          ...
          <!-- $store.getters.checkPermission('kpi-owner') &&
              ...
              !document_details.some((v) => v.document_detail_fakt.status != 1) -->
          <!-- {{document_details.filter(v => !!v.document_detail_fakt.fakt).length}} -->
                ...
                >
                  <v-textarea
                    ...
                    v-if="
                      ...
                      (
                        // change_data || 
                        detail.document_detail_fakt.status == null
                      ) || user.id == 6
                    "
  ...
  methods: {
    ...
    getDepartments(e) {
      axios
        .post(this.$store.state.backend_url + "api/documents/kpi-departments", {
          ...
          quarter: (e == 0) ? 0 : this.filter.quarter,
        })
        .then((res) => {
    ...
    getlist() {
      ...
      if (this.filter.department_id || !this.$store.getters.checkPermission("kpi-comission"))
        axios
          .post(
            this.$store.state.backend_url + "api/documents/get-attributes",
            {
              filter: this.filter,
            }
          )
          .then((response) => {
            if((response.data.doc.length) > 1){
              this.doc_count = true;
