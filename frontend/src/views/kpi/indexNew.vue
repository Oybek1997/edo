  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title>
        ...
        <v-chip :to="'/document/' + doc.pdf_file_name"  target="_blank" class="ml-5 success" v-for="(doc, i) in kpi_documents" :key="i"  small>{{ doc.document_number }}</v-chip>
        <span class="ml-5">
          <i>
            {{ kpi_documents && kpi_documents[0] && kpi_documents[0].employee ?  kpi_documents[0].employee.fio : '' }}
          </i>
          <i>
            {{  kpi_documents && kpi_documents[0] ? ('(' + kpi_documents[0].responsible_contact  + ')') : '' }}
          </i>
        </span>
        <v-spacer></v-spacer>
        <v-toolbar
        ...
        >
          ...
          <span v-if="doc_count" style="max-width: 200px">
            <v-autocomplete
              label="Document"
              v-model="filter.doc_id"
              :items="doc"
              hide-details
              @change="getlist"
              dense
          ...
          {{ '4-'+ saveComission }} -->
          <v-btn
            v-if="(saveComission && !kpi_user) || kpi_view || (saveOwner && kpi_user) || user.id == 6"
            color="success"
            outlined
            @click="save()"
            class="ml-1"
          ...
          user.id == 504 ?
            ...
            s.d_d_attribute_id == attributes[filter.quarter - 1][1]).attribute_value != 0).find((v) =>
            v.document_detail_fakt.comissions.length < 11 ).id : '1'
          }} -->
          <!-- {{ user.id }} -->
          ...
          <v-btn
            v-if="            
              ...
              (kpi_user || kpi_view) && 
              !validateCreateReportDoc &&
              !document_details.filter((v) =>v.document_detail_attribute_values.find((s) => 
            s.d_d_attribute_id == attributes[filter.quarter - 1][1]).attribute_value != 0).some((v) =>
            v.document_detail_fakt.comissions.length < 11 && v.document_detail_fakt.reward_amount !=0 )  
            && 
                ...
                >
                  <v-textarea
                    ...
                    v-if="
                      ...
                      (
                        change_data || 
                        (detail.document_detail_fakt.status && detail.document_detail_fakt.status != 2)
                      ) || user.id == 6
                    "
          ...
          <v-form>
            <v-row>
              ...
              <v-col cols="2" style="min-width: 100px" class="px-0">
                <v-btn
                  :disabled="!selectCommentFiles || selectCommentFiles.length == 0"
                  class="mt-6"
                  color="success"
                  block
                  @click="addCommentFiles"
                >
                  +
                </v-btn>
              </v-col>
    ...
    <v-card v-if="true || change_data || change_reaction" class="ma-1 pa-1">
      ...
      <v-card-title 
        >Muloqot oynasi
        <v-autocomplete
          v-if="change_data || change_reaction"
          label="Ko'rsatkich"
          v-model="d_d_id"
          :items="d_d_name"
          hide-details
      ...
      <v-card-text>
        ...
        >
          ...
          <v-card-text>
            <div
              v-for="(textComment, i) in item.kpi_comments.filter(
                (v) =>
...
export default {
  ...
  // },
    ...
    saveComments() {
      axios
        .post(this.$store.state.backend_url + "api/documents/kpi-comments-new", {
    ...
    save() {
      ...
      // } else
      {
        if (this.kpi_user || this.kpi_view) {
          axios
            .post(this.$store.state.backend_url + "api/documents/save-facts-new", {
              document_details: this.document_details,
              filter: this.filter,
            })
            .then((res) => {
    ...
    getlist() {      
      ...
      this.loading = true;
        axios
          .post(
            this.$store.state.backend_url + "api/documents/get-attributes-new",
            {
              filter: this.filter,
            }
          )
          .then((response) => {
            ...
            // console.log(response.data.kpi_user);
            
            if(response.data.kpi_user){
              
              this.kpi_view = true;
            }
            if((response.data.doc && response.data.doc.length) > 1){
              this.doc_count = true;
              this.doc = response.data.doc;
            } else {
              this.doc_count = false;
            }
            this.d_d_name = [];
