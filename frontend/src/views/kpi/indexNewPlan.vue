  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title>
        ...
        <v-chip v-if="doc" :to="'/documentsidebar/document/' + doc.pdf_file_name"   class="ml-5 success"  small>{{ doc.document_number }}</v-chip>
        <v-spacer></v-spacer>
        <v-toolbar flat dense class="mx-1">
          ...
          <!-- <v-btn v-if="check_true_false || user.id == 16" color="success" outlined @click="save(); check_true_false == false" class="ml-1"> -->
          <v-btn v-if="check_true_false" color="success" outlined @click="save(); check_true_false = false" class="ml-1">
            {{ $t("Saqlash!") }}
          </v-btn>
          ...
          <template v-slot:default>
            ...
            <tbody>
              <tr v-for="(detail, k) in document_details" :key="k">
                ...
                <td v-for="(comission, index) in detail.kpi_plan_comissions" :key="index" width="100" style="border: 1px solid #ddd">
    ...
    <v-card  class="ma-1 pa-1">
      <v-card-title style="padding: 2px 8px;">Yuklangan fayllar
        <v-spacer></v-spacer>     
    ...
    <v-card v-if="true || change_data || change_reaction" class="ma-1 pa-1">
      ...
      <v-card-title>Muloqot oynasi
        <v-autocomplete  label="Ko'rsatkich" v-model="d_d_id" :items="d_d_name"
          hide-details dense outlined item-value="d_id" item-text="name" class="mx-1"
      ...
      <v-card-text>
        <v-card v-for="(item, index) in document_details" :key="index" class="my-2">
          <v-card-title style="font-size: 15px; padding: 2px 8px;">
            {{
              (index+1) + ' ' +
          ...
          <v-card-text>
            ...
            <div v-for="(textComment, i) in item.kpi_comments" :key="i">
              <v-alert border="right" dense outlined color="green" class="my-1">
                <div>
                  <b style="font-size: 15px;" v-if="textComment.employee">
                    {{
                ...
                <div style="font-size: 15px;">
                  {{ textComment.comment }}
                  <v-chip style="font-size: 15px;" small v-for="(file, i) in textComment.files" :key="i">
                    {{ file.file_name }}
...
export default {
  ...
  methods: {
    ...
    changeStatus(comission, status) {
      this.check_true_false = true;
      this.document_details = this.document_details.map((d) => {
        d.kpi_plan_comissions = d.kpi_plan_comissions.map((c) => {
            if (c.employee_id == this.employee.id && c.d_d_id == comission.d_d_id && c.quarter == comission.quarter ) {
              c.status = status;
            }
            return c;
          });
    ...
    save() {
      ...
      axios
        .post(
          this.$store.state.backend_url + "api/documents/save-reactions-plan",
          {
    ...
    getlist() {
      ...
      axios
        ...
        .then(() => {
          if (this.$store.getters.checkPermission("kpi-comission")) {
            this.document_details = this.document_details.map((v) => {
              if (
                this.$store.getters.checkPermission("kpi-comission") &&
                v.kpi_plan_comissions.length < 11 &&
                !v.kpi_plan_comissions.some(
                  (c) => c.employee_id == this.employee.id
                )
              ) {
                v.kpi_plan_comissions.push({
                  comment: null,

                  created_at: new Date().toISOString().slice(0, 16),
                  d_d_id: v.id,
                  // d_d_a_v_id: v.document_detail_attribute_values.find(
                  //     (v) =>
                  //       v.d_d_attribute_id ==
                  //       this.attributes[this.filter.quarter - 1][0]
                  //   ).d_d_attribute_id,
                  quarter : this.filter.quarter,
                  employee: {
                    id: this.employee.id,
              ...
              v.kpi_plan_comissions =
                v.kpi_plan_comissions.sort(
                  (a) => new Date(a.created_at) * -1
                );
              return v;
            });
        ...
        .then(() => {
          this.comissions = this.document_details[0] && this.document_details[0] ? this.document_details[0].kpi_plan_comissions : '';
          this.document_details.forEach((d) => {
            d.kpi_plan_comissions.forEach((c) => {
