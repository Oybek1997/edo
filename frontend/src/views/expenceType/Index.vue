  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("expenceType.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('expence_type-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('expence_type-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('expence_type-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("expence_type-update") ||
          this.$store.getters.checkPermission("expence_type-delete")
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/expence-types", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("expence_type-create")) {
        this.dialogHeaderText = this.$t("expenceType.create");
        this.form = {
          id: Date.now(),
          name: "",
          email: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("personal_type-update")) {
        this.dialogHeaderText = this.$t("expenceType.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/expence-types/update",
            this.form
          )
          .then((res) => {
            this.getList();
