    ...
    <div class="fullHeight">
      ...
      >
        <v-card-title class="px-4 py-3">
          <span class="headerTitle mb-2">{{ $t("Yuklagich haydovchilar") }}</span>
              ...
              >
                <span v-if="item.status == 1">
                  <v-btn x-small color="primary" dark class="tdClass" right>
                    {{ $t("Ayni payitda ishlayapti") }}
                  </v-btn></span
                  >
                  <span v-else-if="item.status == 0" class="indigo">
                    <v-btn x-small color="error" dark class="tdClass" right>
                    {{ $t("Boshqa ishga o‘tib ketgan") }}
                  </v-btn></span
                >
              </p>
            </template>
  ...
  export default {
    data() {
      return {
        ...
        items: [],
        repairplumberstatus: [
          { id: 1, name: "Ayni payitda ishlayapti" },
        { id: 0, name: "Boshqa ishga o‘tib ketgan" },
      ],
        form: {},
        dialogHeaderText: "",
        page: 1,
    ...
    computed: {
      ...
      headers() {
        return [
        {
          ...
          {
            text: this.$t("Bo'lim kodi"),
            class: "blue-grey lighten-5",
            value: "dep_code",
          },
          {
            text: this.$t("Familyasi"),
            class: "blue-grey lighten-5",
            value: "surname",
          },
          {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        axios
          .post(this.$store.state.backend_url + "api/supplytransports/all-repairplumbers", {
      ...
      newItem() {
        this.dialog = true;
          this.dialogHeaderText = this.$t("Trasnport qo'shish");
          this.form;
          this.editMode = false;
      },
      editItem(item) {
          this.dialogHeaderText = this.$t("Transport ma'lumotlarini o'zgaritish");
      ...
      save() {
        axios
          .post(
            this.$store.state.backend_url + "api/saveupdate/repairplumber",
            this.form
          )
          .then((res) => {
            this.getList();
      ...
      deleteItem(item) {
        if (this.$store.getters.checkPermission("organizations-delete")) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              axios
                .post(this.$store.state.backend_url + "api/delete/repairplumber",{
                  id: item.id,
                })
                .then((res) => {
                  this.getList();
