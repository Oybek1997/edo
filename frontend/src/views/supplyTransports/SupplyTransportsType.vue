  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t('Transportlar') }}</span>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("Yangi transport qo'shish") }}</label>
                <v-text-field
                  v-model="form.name"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/supplytransports/all-transportstype", {
    ...
    newItem() {
        this.dialogHeaderText = "Transport qo'shish";
        this.form = {
          name: "",
        };
        this.dialog = true;
    ...
    save() {
        axios
          .post(
            this.$store.state.backend_url + "api/saveupdate/supplytransporttype",
            this.form
          )
          .then(res => {
            this.getList();
            ...
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/delete/supplytransporttype/" +
                  item.id
              )
              .then(res => {
