  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("natioanalities.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('nationality-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn 
                v-if="$store.getters.checkPermission('nationality-update')"
                class="px-0"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('nationality-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px" small text @click="deleteItem(item)">
                <v-icon size="18">mdi-detrash-can-outlinelete</v-icon>
              </v-btn>
            </template>
          </v-data-table>
        </v-col>
      ...
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="dialogTitle">{{
            $t("natioanalities.dialogHeaderText")
          }}</span>
        </v-card-title>
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/nationality")
        .then((response) => {
    ...
    newItem() {
      this.dialogHeaderText = this.$t("nationality.newNatioanality");
      this.form = {
        id: Date.now(),
        country_id: "",
        name_uz_latin: "",
    ...
    editItem(item) {
      this.dialogHeaderText = this.$t("nationality.newNatioanlity");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/nationality/update",
            this.form
          )
          .then((res) => {
            this.getList();
