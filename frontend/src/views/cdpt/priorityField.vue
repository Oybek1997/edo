  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        <span>{{ $t("cdpt.priority_field") }}</span>
        <v-spacer></v-spacer>
        <v-btn
      ...
      >
        ...
        <template v-slot:item.sequence="{ item }">
          <template v-if="item.sequence == 1">1-Yo'nalish</template>
          <template v-else-if="item.sequence == 2">2-Yo'nalish</template>
          <template v-else-if="item.sequence == 3">3-Yo'nalish</template>
        </template>
...
export default {
  data() {
    return {
      ...
      sequences: [
        { value: 1, text: "1-Yo'nalish" },
        { value: 2, text: "2-Yo'nalish" },
        { value: 3, text: "3-Yo'nalish" }
      ],
      dialogHeaderText: "",
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("cdpt.sequence"), value: "sequence" },
        {
          text: this.$t("actions"),
          value: "actions",
          width: 120,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/cdpt/priority-fields", {
    ...
    newItem() {
      ...
      this.dialogHeaderText = this.$t("cdpt.add_priority_field");
      this.form = {
        id: Date.now()
      };
      this.dialog = true;
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/cdpt/priority-fields/update",
            this.form
          )
          .then(res => {
            this.getList();
      ...
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete(
              this.$store.state.backend_url +
                "api/cdpt/priority-fields/delete/" +
                item.id
            )
            .then(res => {
