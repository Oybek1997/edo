  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("object_type.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('object_type-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('object_type-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('object_type-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>Controller</label>
                <v-text-field
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/object-types", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("object_type-create")) {
        this.dialogHeaderText = this.$t("object_type.create");
        this.form = {
          id: Date.now(),
          controller: "",
        };
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("object_type-update")) {
        this.dialogHeaderText = this.$t("object_type.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/object-types/update",
            this.form
          )
          .then((res) => {
            this.getList();
