  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        <span>{{ $t("attestation.at_commissions") }}</span>
        <v-spacer></v-spacer>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save">
            <v-row>
              <v-col cols="12">
                <label for>{{ $t("attestation.commission_number") }}</label>
                <v-text-field
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("attestation.commission_number"),
  ...
  methods: {
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/attestation-commissions")
        .then(response => {
    ...
    newItem() {
      this.dialog = true;
      this.dialogHeaderText = this.$t("attestation.add_commission");
      this.form = {
        id: Date.now(),
        commission_number: "",
        description: ""
    ...
    save() {
      axios
        .post(
          this.$store.state.backend_url + "api/attestation-commissions/update",
          this.form
        )
        .then(res => {
          this.getList();
          ...
          axios
            .delete(
              this.$store.state.backend_url +
                "api/attestation-commissions/delete/" +
                item.id
            )
            .then(res => {
              this.getList();
