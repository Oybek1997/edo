  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("inventory.address_product") }}</span>
          ...
          >
            <template v-slot:[`body.prepend`]>
              <tr class="py-0 my-0 prepend_height">
                ...
                <td class="py-0 my-0 dense">
                  <v-text-field
                    v-model="filterForm.address_name"
                    type="text"
                    hide-details
                    @keyup.native.enter="getList()"
                  ></v-text-field>
            ...
            <template v-slot:item.warehouse="{ item }">
              <span style="white-space: normal; max-width: 100px">
                {{ item.address && item.address.warehouse ? item.address.warehouse.wh_number : "" }}
              </span>
            </template>
            <template v-slot:item.address_name="{ item }">
              <span style="white-space: normal; max-width: 100px">
                {{ item.address && item.address.address_name ? item.address.address_name : "" }}
              </span>
            </template>
            <template v-slot:item.product_name="{ item }">
              <span style="white-space: normal; max-width: 100px">
                {{
                  item.product && item.product.part_number ? item.product.part_number : ""
                }}
              </span>
            </template>
          </v-data-table>
    ...
    >
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="dialogTitle">{{ $t("inventory.address") }}</span>
        </v-card-title>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("inventory.addressName") }}</label>
                <v-text-field
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("inventory.warehouse") }}</label>
                <v-autocomplete
                  clearable
                ...
                >                
                  <template slot="item" slot-scope="{ item }">
                    {{ item ? item.wh_name + "  " + item.wh_number + " " : "" }}
                  </template>
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("inventory.warehouse"), value: "warehouse" },
        { text: this.$t("inventory.addressName"), value: "address_name" },

        // { text: this.$t("wh_number"), value: "wh_number" },

        { text: this.$t("inventory.productName"), value: "product_name" },
        { text: this.$t("inventory.stock"), value: "stock" },
  ...
  methods: {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(this.$store.state.backend_url + "api/pg_inventory/adress/update", this.form)
          .then((res) => {
            this.getList();
            this.dialog = false;
    ...
    deleteItem(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete(
              this.$store.state.backend_url + "api/pg_inventory/adress/delete/" + item.id
            )
            .then((res) => {
    ...
    warehouse() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/pg_inventory/getwh", {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/pg_inventory/address-product", {
          filter: this.filterForm,
          search: this.search,
          pagination: this.dataTableOptions,
        })
