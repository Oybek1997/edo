  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("inventory.product") }}</span>
      ...
      <v-row class="mx-0">
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            <template v-slot:[`body.prepend`]>
              <tr class="py-0 my-0 prepend_height">
                ...
                <td class="py-0 my-0 dense">
                  <v-text-field
                    v-model="filterForm.commission_number"
                    type="text"
                    hide-details
                    @keyup.native.enter="getList()"
                  ></v-text-field>
            ...
            <template v-slot:item.commission_number="{ item }">
              <span style="white-space: normal; max-width: 100px">
                {{ item.commission_number }}
              </span>
            </template>
    ...
    >
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="dialogTitle">{{ $t("inventory.product") }}</span>
        </v-card-title>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("inventory.productName") }}</label>
                <v-text-field
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("inventory.productName"), value: "commission_number" },
  ...
  methods: {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(this.$store.state.backend_url + "api/pg_inventory/commission/update", this.form)
          .then((res) => {
            this.getList();
            this.dialog = false;
    ...
    deleteItem(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete(
              this.$store.state.backend_url + "api/pg_inventory/commission/delete/" + item.id
            )
            .then((res) => {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/pg_inventory/commission", {
          filter: this.filterForm,
          search: this.search,
          pagination: this.dataTableOptions,
        })
