  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("inventory.warehouse") }}</span>
      ...
      <v-row class="mx-0">
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.wh_name="{ item }">
    ...
    >
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="dialogTitle">{{ $t("inventory.warehouse") }}</span>
        </v-card-title>
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("inventory.warehouse_number") }}</label>
                <v-text-field
                  v-model="form.wh_number"
                  :rules="[(v) => !!v || $t('input_required')]"
                  hide-details="auto"
                  dense
                  outlined
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("inventory.warehouse"), value: "wh_name" },

        { text: this.$t("inventory.warehouse_number"), value: "wh_number" },
  ...
  methods: {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(this.$store.state.backend_url + "api/pg_inventory/wh/update", this.form)
          .then((res) => {
            this.getList();
            this.dialog = false;
    ...
    deleteItem(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete(
              this.$store.state.backend_url + "api/pg_inventory/wh/delete/" + item.id
            )
            .then((res) => {
