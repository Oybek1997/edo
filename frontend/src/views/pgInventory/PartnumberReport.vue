  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("Detal raqami bo'yicha hisobot") + " " }} </span>
      ...
      <v-card class="pa-5">
        <v-card-title class="py-1 px-3">
          ...
          >
            <download-excel
              :data="excel"
              :name="'Partnumber_report' + today + '.xls'"
            >
...
export default {
  ...
  methods: {
    ...
    getExcel() {
      ...
      axios
        .post(
          this.$store.state.backend_url + "api/pg_inventory/get-partnumber-report",
          {
            filter: this.filter,
            excel : 1
          }
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/pg_inventory/get-partnumber-report",
        {
          filter: this.filter
        })
        .then((response) => {
