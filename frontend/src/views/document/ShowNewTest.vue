      ...
      <v-card-title class="pa-0 pb-1" style="color: #000" color="black">
        ...
        <v-btn
          v-if="
            ...
            (document.status == 0 ||
              (document.status == 7 &&
                !document.document_signers.filter((v) => {
                  if (v.status == 5) return v;
                }).length))
          "
          small
          outlined
        ...
        <v-btn
          ...
          icon
          :to="'/documents/show-only-pdf/' + document.id"
          color="blue"
          :title="$t('old_version')"
        >
        ...
        <v-btn icon @click="star()" color="green" :title="$t('refresh_pdf')">
          ...
          <v-col cols="12" lg="3" v-if="rightMenu" class="pa-0">
            ...
            >
                  ...
                  <template v-for="(edit_attribute, index) in edit_attributes">
                    <v-card outlined :key="index">
                      <v-system-bar color="grey lighten-4">
                        ...
                        >{{ edit_attribute.document_detail_employees[0].employee_fio }}</span>
                        <span
                          v-if="
                                ...
                                :readonly="
                                  !(
                                    edit_attributes.some((e) =>
                                      e.document_detail_signer_attributes.some(
                                        (a) =>
                                          user.employee.employee_staff.find(
                                            (v) =>
                                              a.attribute_signer_staff.find(
                                                (va) =>
                                                  v.staff_id == va.staff_id
                                              )
                                          )
                                      )
                                    ) && document.reaction_show
              ...
              <v-card
                outlined
                class="mb-2"
                v-if="document.resolution_show || document.action_type_id == 5 || document.action_type_id == 11"
              >
                <v-system-bar
                  class="pa-1 justify-center"
                  style="background-color: #163e72; color: #fff"
              ...
              <!-- bajarubchilar ro'yxati nazotarchi uchun -->
                      ...
                      <tbody>
                        <tr v-for="(item, index) in document.document_relation" :key="index">
                          ...
                          <td>
                            ...
                            <!-- v-if="$store.getters.checkRole('superadministrator')" -->
                            <router-link
                              v-if="$store.getters.checkPermission('all-document-show')"
                          ...
                          <v-row :key="documentSigner.index" class="ma-0 justify-space-between">
                            ...
                            <v-col class="pa-0 text-right">
                              <v-chip
                                ...
                                small
                                color="white"
                              >{{ documentSigner.signed_at }}</v-chip>
                              <v-btn
                                icon
                                small
                                :color="
              ...
              <v-card outlined class="mb-2" v-if="document.staff">
                ...
                <v-card-text class="pa-2 pb-1">
                  {{
                  document.staff['department_code']+' '+document.staff['department_name_'+$i18n.locale]
                  }}
                  <br />
                  {{
              ...
              <v-card outlined class="mb-2" v-if="document.department2">
                ...
                <v-card-text class="pa-2 pb-1">
                  {{
                  document.department2['department_code']+' ' +document.department2['department_name_'+$i18n.locale]
                  }}
                  <br />
                  {{
            ...
            <v-row>
              ...
              <v-col cols="4" md="4" class="pa-1">
                <v-autocomplete
                  ...
                  :items="
                    resolutionTypes.filter((v) => {
                      if (v.is_resolution == 1) return v;
                    })
                  "
                  item-value="id"
                  :rules="[(v) => !!v || $t('input.required')]"
...
export default {
  ...
  methods: {
    ...
    getNumber() {
      this.loading = true;
      if (this.document.document_number != "000000") {
        this.loading = false;
        this.sign(1);
      } else
        axios
    ...
    getList(refreshPdf) {
      ...
      axios
        .post(
          this.$store.state.backend_url +
            "api/documents/show_new_test-document",
          {
