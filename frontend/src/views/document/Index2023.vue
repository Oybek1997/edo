  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        ...
        <span v-else-if="filter.menu_item == 'akt'">{{
          $t("AKT")
        }}</span>
        <span v-else-if="filter.menu_item == 'akt-cancel'">{{
          $t("Cancelled Acts")
        }}</span>
        <span v-else-if="filter.menu_item == 'in_out'">{{ $t("in_out") }}</span>
        <span v-else-if="filter.menu_item == 'annulirovan'">{{ $t("annulirovan") }}</span>
      ...
      <!-- START DOCUMENTS SIMPLE TABLE -->
      <v-simple-table class="ma-1 mainTable" id="table" dense fixed-header
      :height="screenHeight">
        <thead>
          <tr  style="background-color: #000000 !important;">
            <template v-for="(header, index) in headers">
              <th style="font-weight: bold; color:#000000" class="my-header-class">{{ header.text }}</th>
            </template>
          </tr>
        </thead>
        <tbody>
      ...
      <v-row class="my-0">
        ...
        <v-col xl="1" lg="1" md="1" style="max-width: 10%;">
          <v-select
        ...
        <v-col xl="2" lg="3" md="1" style="max-width: 20%;">
          <v-btn
            icon 
...
export default {
  ...
  computed: {
    ...
    headers() {
      ...
      filtered_headers = filtered_headers.filter((header) => {
        if (this.$route.params.menu_item != 'akt') {
          return (
            header.value != "act_date"
          );
        } else return filtered_headers;
      });
  ...
  methods: {
    ...
    getList2() {
      ...
      this.noDataText = this.$t("loadingText");
      axios
        .get(this.$store.state.backend_url + "api/document-report/inbox/1"
        )
        .then((response) => {
