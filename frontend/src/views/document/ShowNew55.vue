    ...
    <v-card class="ma-1 pa-1 mr-0" style="overflow: hidden">
      <v-card-title class="pa-0 pb-1" style="color: #000" color="black">
        ...
        <v-btn
          ...
          title="Download pdf with comments."
        >
        ...
        <v-btn
          v-if="
            $store.getters.checkPermission('annulirovan') /*&& document.document_template && 
            document.document_template.annulirovan*/ && (document.status == 3 || document.status == 4 || document.status == 5)
          "
          small
          outlined
          width="220px"
        ...
        <v-row class="justify-center px-2" :height="screenHeight">
          ...
          <v-col cols="12" lg="3" v-if="rightMenu" class="pa-0">
            ...
            >
                  ...
                  <template v-for="(control_punkt, cp_index) in control_punkts">
                    ...
                    >
                      <v-system-bar color="grey lighten-4">{{
                        $t("control_punkt.name") + " " + (cp_index + 1)
                      }}</v-system-bar>
                      <v-card-text class="pa-1">{{
              ...
              <!-- edit attributes -->
              <v-card outlined class="mb-2" v-if="edit_attributes.length && document.action_type_id == 4">
                <v-system-bar
                  class="pa-1 justify-center"
                  style="background-color: #163e72; color: #fff"
                  elevation="3"
                    ...
                    <v-card outlined :key="documentDoer.index" class="mb-2">
                      ...
                      >
                        {{
                          documentDoer.fio
                            ? documentDoer.fio
                            : documentDoer.staff.employees[0] ? documentDoer.staff.employees[0][
                                "firstname_" + language
                              ].substr(0, 1) +
                              "." +
              ...
              <v-card
                v-if="documentFiles && documentFiles.filter(v=>v.object_type_id==5).length"
                class="mb-2"
                outlined
              >
                <v-system-bar
                ...
                <v-card-text class="ma-0 pa-1" v-if="active_files">
                  ...
                  >
                    <template v-slot:default>
                      <tbody>
                        <tr v-for="(item, index) in documentFiles.filter(v=>v.object_type_id==5)" :key="index">
              ...
              <!-- GMK Invoice fayllar -->
              <v-card
                v-if="document.id==2446936 && documentFiles && documentFiles.filter(v=>v.object_type_id==15).length"
                class="mb-2"
                outlined
              >
                <v-system-bar
                ...
                <v-card-text class="ma-0 pa-1" v-if="active_files">
                  ...
                  >
                    <template v-slot:default>
                      <tbody>
                        <tr v-for="(item, index) in documentFiles.filter(v=>v.object_type_id==15)" :key="index">
                          <td style="width: 100%; padding-left: 4px">
                            <v-chip outlined small @click="viewPdfFile(item)">{{
                              item.file_name
                            }}</v-chip>
                    ...
                    <v-card-text class="pa-1">
                      ...
                      >
                        <v-chip
                          ...
                          small
                          :outlined="
                            control_punkt.controller.status == 0 ? true : false
                          "
                          :color="
                            control_punkt.controller.status == 1
                              ? 'green lighten-4'
                        ...
                        >
                          ...
                          <v-icon v-if="control_punkt.controller.description"
                            >mdi-dots-horizontal</v-icon
                          >
                        </v-chip>
                        <template slot="popover">
                          ...
                          >
                                ...
                                {{
                                  control_punkt.controller.fio
                                    ...
                                    : control_punkt.controller.staff.employees[0][
                                      ...
                                      ". " +
                                      control_punkt.controller.staff
                                        .employees[0]["lastname_" + language]
                                }}
                              </v-system-bar>
                              <v-card-text style="color: #000; padding: 5px">
                            ...
                            <v-card
                              outlined
                              tile
                              v-if="control_punkt.controller.taken_datetime"
                            >
                              <v-card-text style="color: #000; padding: 5px">
                                <v-chip
                                  class="mx-1"
                                ...
                                >
                                  <v-icon left>mdi-check-bold</v-icon>
                                  {{
                                    control_punkt.controller.updated_at.substr(
                                      0,
                                      10
                                    ) +
                                    " " +
                            ...
                            >
                              ...
                              <v-card-text style="color: #000; padding: 5px">{{
                                control_punkt.controller.description
                              }}</v-card-text>
                            </v-card>
                          </v-card>
                        </template>
                    ...
                    <v-card-text class="pa-1">
                      ...
                      >
                        <template v-slot:label="{ item }">
                          <v-popover
                            bottom
                            v-if="item.fio || item.staff.employees.length"
                          >
                            <v-chip
                              label
                              small
        ...
        <v-card-text>
          <v-form name="testform" ref="reactionForm">
            ...
            <v-row>
              ...
              <v-col cols="12" v-if="document.document_template_id>487&&document.document_type_id==12">
                <label for>{{ $t("Akt oyi") }}</label>
                <v-select
                  :items="months"
              ...
              <v-col cols="12">
                <v-card outlined>
                  <v-card-title class="grey lighten-4 py-0">{{
                    $t("control_punkt.distribution_list")
                  }}</v-card-title>
...
export default {
  ...
  methods: {
    ...
    meningHujjatimmas(message) {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/documents/mening-hujjatim-emas", {
          document_id: this.document.id,
          message: message,
        })
        .then((res) => {
    ...
    editControlPunkt(item) {
      ...
      this.control_punkt.document_signers = this.document_signers.filter(
        (v) => {
    ...
    deleteControlSigner(item) {
      this.control_punkt.document_signers =
        this.control_punkt.document_signers.filter((v) => {
          if (v.id != item.id) {
            return v;
          }
        });
    ...
    publish() {
      this.loading = true;
      if(this.document.document_type_id == 12 && this.document.document_template_id>487){
