<template>
  <div>
    <v-card class="ma-1 pa-1 mr-0 heightFull" style="overflow: hidden">
      <v-card-title class="pa-0 pb-1" style="color: #000" color="black">
        ...
        <v-tooltip bottom>
          ...
          <span>{{ $t("To`liq oynada ko`rish") }}</span>
        </v-tooltip>
        <!-- <v-btn icon @click="fullScreenDialog = true" :title="$t('To`liq oynada ko`rish')">
        ...
        <v-tooltip bottom>
          ...
          <span>{{ $t("Xujjatni yuklab olish") }}</span>
        </v-tooltip>
        <!-- <v-btn icon @click="downloadPdf" :title="$t('Xujjatni yuklab olish')">
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              ...
              v-on="on"
              icon
              @click="downloadPdfWithComment"
            >
              <v-icon color="grey">mdi-download-circle-outline</v-icon>
            </v-btn>
          </template>
          <span>{{ $t("Xujjatni izohlari bilan yuklab olish") }}</span>
        </v-tooltip>
        <!-- <v-btn
          icon
          @click="downloadPdfWithComment"
          title="Xujjatni izohlari bilan yuklab olish"
        ...
        <v-tooltip bottom color="success">
          ...
          <span>{{ $t("Xujjat sarlavhasini o`zgartirish") }}</span>
        </v-tooltip>
        <!-- <v-btn
          v-if="$store.getters.checkPermission('okd_kanselyariya')"
          icon
          @click="dialogEditDocumentTitle = true"
          color="success"
          :title="$t('Xujjat sarlov xasini o`zgartirish')"
          >
        ...
        <!-- <v-btn
          ...
          document.document_signers.some((v) => v.status == 2))
          "
          icon
        ...
        <v-tooltip bottom color="success">
          ...
          <span>{{ $t("Hujjatni qayta yuklash") }}</span>
        </v-tooltip>
        <!-- <v-btn
          icon
          @click="getList(true)"
          color="green"
          :title="$t('Hujjatni qayta yuklash')"
          >
          <v-icon>mdi-refresh</v-icon>
        </v-btn> -->
        <v-tooltip bottom color="#3FCB5D">
          <template v-slot:activator="{ on, attrs }">
            <v-btn v-bind="attrs" v-on="on" icon @click="star()" color="green">
              <v-icon v-if="isStar == 1">mdi-star</v-icon>
              <v-icon v-else>mdi-star-outline</v-icon>
            </v-btn>
          </template>
          <span>{{ $t("Tanlangan deb xisoblash") }}</span>
        </v-tooltip>
        <!-- <v-btn icon @click="star()" color="green" :title="$t('Tanlangan deb xisoblash')">
          <v-icon v-if="isStar == 1">mdi-star</v-icon>
          <v-icon v-else>mdi-star-outline</v-icon>
        </v-btn> -->
        <v-tooltip bottom color="#3FCB5D">
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              ...
              v-if="
                document &&
                !document.stamped &&
                $store.getters.checkPermission('shtamp_pdf') &&
                [305, 333, 357].includes(document.document_template_id)
              "
            >
        ...
        <v-tooltip bottom color="success">
          ...
          <span>{{ $t("Imzolovchilar guruhini o`zgartirish") }}</span>
        </v-tooltip>
        <v-tooltip bottom color="#3FCB5D">
          <template v-slot:activator="{ on, attrs }">
            <!-- v-if="
        ...
        <!-- <v-btn
          ...
          @click="editSigners"
          color="success"
          :title="$t('Imzolovchigan guruxiga o`zgartirish')"
        >
          <v-icon>mdi-account-edit-outline</v-icon>
        </v-btn> -->
        <v-spacer></v-spacer>
        <v-btn v-if="numPages" icon @click="zoomPdf(0)" fab x-small>
        ...
        <v-btn v-if="numPages" icon @click="zoomPdf(1)" fab x-small>
          <v-icon>mdi-plus</v-icon>
        </v-btn>
        <span fab style="left: 15px; font-size: 12px">{{ zoom }}%</span>
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              v-bind="attrs"
          ...
          <span>{{ $t("Hujjat ko`rinishini qaytarish") }}</span>
        </v-tooltip>
        <v-spacer></v-spacer>
        <v-btn
          v-if="
            (user && user.username == 'qg9592') ||
            ($store.getters.checkPermission('pomoshnik') &&
              [25, 214, 357, 71, 305, 333, 218].includes(
        ...
        <v-btn
          v-if="
            ...
            document.document_template.annulirovan*/ &&
            (document.status == 3 ||
              document.status == 4 ||
              document.status == 5)
          "
        ...
        <v-btn
          ...
          color="success"
          class="ma-1"
          @click="validateEditAttributeForm()"
        >
        ...
        <v-btn
          v-if="
            ...
            (document.status == 0 ||
              (document.status == 7 &&
                !document.document_signers.filter((v) => v.status == 5).length))
          "
          small
          outlined
          width="200px"
        ...
        <v-btn
          v-if="
            (user &&
              ...
              (document.status == 0 ||
                (document.status == 7 &&
                  !document.document_signers.filter((v) => v.status == 5)
                    .length) ||
                ($store.getters.checkPermission('okd_kanselyariya') &&
                  document.document_template_id != 432 &&
                  (document.status == 1 || document.status == 2)))) ||
        ...
        <v-btn
          v-if="
            ...
            document.reaction_show &&
            document.action_type_id != 11 &&
            (document.document_type_id != 72 || document.sequence != 0)
          "
          small
          outlined
          width="150px"
        ...
        <!-- <v-btn
          v-if="
            document.reaction_show &&
            document.action_type_id == 4 &&
            document.document_type_id == 72 &&
            document.sequence == 0 &&
            document.status == 3
          "
          small
        ...
        <v-btn
          ...
          @click="outOnControl()"
        >
          <span style="white-space: normal; width: 220px">
            {{ $t("document.out_on_control") }}
          </span>
        </v-btn>
        <v-btn
        ...
        <v-btn
          v-if="
            ...
            document.reaction_show &&
            document.action_type_id != 11 &&
            (![3, 4, 5].includes(document.status) ||
              [11705, 8928, 8916, 11888].includes(user.employee_id) ||
              document.action_type_id == 8 ||
              ![70, 71, 75, 305, 214, 357, 136, 139, 155, 172].includes(
                document.document_template_id
              ))
          "
          small
          ...
          color="warning"
          class="ma-1"
          @click="modalDocumentSubstantiate = true"
        >
          <span style="white-space: normal; width: 180px">{{
            $t("substantiate")
          }}</span>
        ...
        <v-btn
          v-if="
            ...
            document.action_type_id != 11 &&
            (![3, 4, 5].includes(document.status) ||
              [11705, 8928, 8916, 11888, 918].includes(user.employee_id) ||
              document.action_type_id == 8 ||
              ![70, 71, 75, 305, 214, 357, 136, 139, 155, 172, 373].includes(
                document.document_template_id
              ))
          "
          small
      ...
      <v-card-text class="pa-1" style="color: #000">
        <v-row class="justify-center px-2" :height="screenHeight">
                ...
                <v-card outlined class="ma-1">
                  ...
                  >
                    ...
                    <v-btn icon small @click="active_history = !active_history">
                      <v-icon class="ma-0" color="white" v-if="active_history"
                      ...
                      <template v-slot:label="{ item }">
                        <div class="historyList">
                          <div class="historyAvatar">
                            <div class="no-img" v-if="item.signer_employee_id">
                              {{ item.fio && item.fio.charAt(0) }}
                            </div>
                          </div>
          ...
          <v-col v-if="rightMenu" class="pa-0">
            <v-card
            ...
            >
              <v-card outlined class="mb-2">
                ...
                <v-row
                  ...
                  v-if="
                    ...
                    document.document_details[0].document_detail_contents &&
                    document.document_details[0].document_detail_contents[7] &&
                    document.document_details[0].document_detail_contents[7]
                      .d_d_attribute_id == 1171
                  "
                >
                  <v-col class="ma-0 pa-0">
                ...
                <v-row
                  ...
                  v-if="
                    ...
                    document.document_details &&
                    document.document_details[0].document_detail_contents &&
                    document.document_details[0].document_detail_contents[10] &&
                    document.document_details[0].document_detail_contents[10]
                      .d_d_attribute_id == 1170
                  "
                >
                  <v-col class="ma-0 pa-0">
                    <strong>{{ $t("document.send_method") }}:</strong>
                ...
                >
                  ...
                  <router-link
                    ...
                    >{{
                      document.previous_version.document_number
                    }}</router-link
                  >
                </v-system-bar>
                <v-divider></v-divider>
              ...
              <v-card outlined class="mb-2" v-if="document.resolution">
                ...
                >
                  ...
                  >
                    {{
                      document.resolution.parent_employee[
                        "lastname_" + locale
                      ] +
                      " " +
                      document.resolution.parent_employee[
              ...
              <v-card
                ...
                class="mb-2"
                v-if="
                  edit_attributes.length &&
                  [1, 4].includes(document.action_type_id)
                "
              >
                <v-system-bar
                  class="pa-1 justify-center"
                ...
                >
                  ...
                  <v-btn
                    icon
                    small
                    @click="active_edit_attribute = !active_edit_attribute"
                  >
                    <v-icon
                      class="ma-0"
                      color="white"
                      v-if="active_edit_attribute"
                      >mdi-menu-down</v-icon
                    >
                    <v-icon class="ma-0" color="white" v-else
                      >mdi-menu-up</v-icon
                  ...
                  <template v-for="(edit_attribute, index) in edit_attributes">
                    <v-card outlined :key="index">
                      <v-system-bar color="grey lighten-4">
                        ...
                        <span
                          v-if="
                            edit_attribute.document_detail_employees.length > 1
                          "
                          >...</span
                        >
                      </v-system-bar>
                            ...
                            <v-col cols="12" class="pa-0">
                              <v-form ref="editAttributeForm">
                                <v-autocomplete
                                  ...
                                  :readonly="
                                    !(
                                      user.employee.employee_staff.some((es) =>
                                        document_detail_signer_attribute.attribute_signer_staff
                                          .map((ass) => ass.staff_id)
                                          .includes(es.staff_id)
                                      ) && document.reaction_show
                                    )
                                  "
                                  :label="
                                  ...
                                  "
                                  v-model="
                                    document_detail_signer_attribute.value
                                  "
                                  @keyup="
                                    getTableList(
                                      att_index + '_' + 0,
                                ...
                                <v-textarea
                                  ...
                                  :rules="
                                    ...
                                    document.reaction_show &&
                                    document_detail_signer_attribute
                                      .document_detail_attributes.required
              ...
              >
                ...
                >
                  ...
                  <v-btn
                    icon
                    small
                    @click="active_resolution = !active_resolution"
                  >
                    <v-icon class="ma-0" color="white" v-if="active_resolution"
                      >mdi-menu-down</v-icon
                    >
                    <v-icon class="ma-0" color="white" v-else
                      >mdi-menu-up</v-icon
                ...
                <v-card-text class="pa-1" v-if="active_resolution">
                  <template v-for="resolutionEmployee in resolutionEmployees">
                    ...
                    >
                      ...
                      <v-card-text class="pa-1">
                        ...
                        <v-divider
                          v-if="resolutionEmployee.assignment"
                        ></v-divider>
                        <p v-if="resolutionEmployee.description" class="my-1">
                          - {{ resolutionEmployee.description }}
                        </p>
                      </v-card-text>
                    </v-card>
                  </template>
                  <v-card
                    outlined
                    class="mb-2"
                    v-if="resolutionEmployees.length == 0"
                  >
              ...
              <v-card outlined v-if="this.watcher" class="mb-2">
                ...
                >
                  ...
                  <v-btn icon small @click="active_watcher = !active_watcher">
                    <v-icon class="ma-0" color="white" v-if="active_watcher"
                      >mdi-menu-down</v-icon
                    >
                    <v-icon class="ma-0" color="white" v-else
                      >mdi-menu-up</v-icon
                  ...
                  >
                    <v-card outlined :key="documentDoer.index" class="mb-2">
                      ...
                      >
                        ...
                        <v-btn
                          v-if="
                            documentDoer.status == 1 || documentDoer.status == 2
                          "
                          @click="
                            modalToReturn = true;
                      ...
                      <v-card-text class="pa-1">
                        ...
                        <p v-if="documentDoer.description" class="my-1">
                          - {{ documentDoer.description }}
                        </p>
                      </v-card-text>
                    </v-card>
                  </template>
              ...
              >
                ...
                >
                  ...
                  <v-btn
                    icon
                    small
                    @click="active_pending_action = !active_pending_action"
                  >
                    <v-icon
                      class="ma-0"
                      color="white"
                      v-if="active_pending_action"
                      >mdi-menu-down</v-icon
                    >
                    <v-icon class="ma-0" color="white" v-else
                      >mdi-menu-up</v-icon
                        ...
                        >
                          ...
                          <td>
                            <router-link
                              v-if="
                                $store.getters.checkPermission(
                                  'view_document_with_link'
                                )
                              "
                              :to="
                                '/documents/show-only-pdf/' + item.pdf_file_name
                              "
                              target="_blank"
                          ...
                          <td>
                            <router-link
                              v-if="
                                $store.getters.checkPermission(
                                  'view_document_with_link'
                                ) || [2301008, 2296509].includes(document.id)
                              "
                              :to="
                                '/documents/show-only-pdf/' + item.pdf_file_name
                              "
              ...
              <v-card
                v-if="
                  documentFiles &&
                  documentFiles.filter((v) => v.object_type_id == 5).length
                "
                class="mb-2"
                outlined
              >
                ...
                <v-card-text class="ma-0 pa-1" v-if="active_files">
                  ...
                  >
                    <template v-slot:default>
                      <tbody>
                        <tr
                          v-for="(item, index) in documentFiles.filter(
                            (v) => v.object_type_id == 5
                          )"
                          :key="index"
                        >
              ...
              <v-card
                v-if="
                  document.id == 2446936 &&
                  documentFiles &&
                  documentFiles.filter((v) => v.object_type_id == 15).length
                "
                class="mb-2"
                outlined
              >
              ...
              >
                ...
                >
                  ...
                  <v-btn icon small @click="active_blanks = !active_blanks">
                    <v-icon class="ma-0" color="white" v-if="active_blanks"
                      >mdi-menu-down</v-icon
                    >
                    <v-icon class="ma-0" color="white" v-else
                      >mdi-menu-up</v-icon
                ...
                <v-card-text class="ma-0 pa-1" v-if="active_blanks">
                  ...
                  >
                    ...
                    >
                      <div
                        v-if="document_detail.document_detail_employees.length"
                      >
                        <div
                          v-for="(
                            employee, emp_index
              ...
              <v-card outlined class="mb-2">
                ...
                >
                  ...
                  <v-btn icon small @click="active_sign = !active_sign">
                    <v-icon class="ma-0" color="white" v-if="active_sign"
                      >mdi-menu-down</v-icon
                    >
                    <v-icon class="ma-0" color="white" v-else
                      >mdi-menu-up</v-icon
                  ...
                  <template v-for="action_type in action_types">
                    <v-card
                      ...
                      v-if="
                        ...
                        documentSigners.find((v) => {
                          if (
                            action_type.id == v.action_type_id &&
                            !v.control_punkt_id
                          )
                            return true;
                          else false;
                    ...
                    >
                      <v-system-bar color="grey lighten-4">{{
                        action_type["name_" + $i18n.locale]
                      }}</v-system-bar>
                      <v-card-text class="pa-1">
                        <template
                          v-for="documentSigner in documentSigners.filter(
                            (v) => {
                              if (
                                action_type.id == v.action_type_id &&
                                !v.control_punkt_id
                            ...
                            <v-col class="pa-0">
                              ...
                              >
                                <v-chip label small color="white">
                                  ...
                                  <v-icon v-if="documentSigner.description"
                                ...
                                <template slot="popover">
                                  ...
                                  >
                                    ...
                                    <v-card
                                      outlined
                                      tile
                                      v-if="documentSigner.taken_datetime"
                                    >
                                      <v-card-text
                                        style="color: #000; padding: 5px"
                                      >
                                        ...
                                        >
                                          <v-icon left>mdi-check-bold</v-icon>
                                          {{
                                            documentSigner.updated_at.substr(
                                              0,
                                              10
                                            ) +
                                            " " +
                              ...
                              <v-alert type="error" v-else>
                                Shtat topilmadi. {{ documentSigner.fio }}.
                                Signer ID{{ documentSigner.id }}
                              </v-alert>
                            </v-col>
                            <v-col class="pa-0 text-right">
                              <v-chip
                                v-if="documentSigner.status"
                                small
                                color="white"
                              >
                                {{ documentSigner.signed_at }}
                              ...
                              >
                                <template v-if="documentSigner.status == 1">
                                  <v-icon v-if="documentSigner.sign_type == 0"
                                ...
                                <v-icon v-else-if="documentSigner.status == 3"
                                  >mdi-timer-sand</v-icon
                                >
                                <v-icon v-else-if="documentSigner.status == 4"
              ...
              <v-card
                ...
                v-if="
                  document &&
                  document.document_staff &&
                  document.document_staff.length
                "
              >
                <v-system-bar
                  class="pa-1 justify-center"
              ...
              <!-- Document Attributes -->
              <v-card
                v-if="[305, 357].includes(document.document_template_id)"
                class="mb-2"
                outlined
              >
                <v-system-bar
                  ...
                  elevation="3"
                >
                  <v-spacer></v-spacer>{{ $t("profile.doc_attributes")
                  }}<v-spacer></v-spacer>
                  <v-btn
                    icon
                    small
                    @click="active_attributes = !active_attributes"
                  >
                    <v-icon class="ma-0" color="white" v-if="active_attributes"
                      >mdi-menu-down</v-icon
                    >
                    <v-icon class="ma-0" color="white" v-else
                      >mdi-menu-up</v-icon
                ...
                <v-card-text class="ma-0 pa-1" v-if="active_attributes">
                  <!-- <label class="ml-2">{{ $t("Attributelar") }}</label> -->
                  <v-simple-table :disable-pagination="true" dense>
                    <template v-slot:default>
                      <tbody>
                        <tr
                          v-for="(item, index) in document.document_details[0]
                            .document_detail_contents"
                          :key="index"
                        >
                          <td
                            style="
      ...
      <v-card>
        <v-card-title class="grey lighten-3">
          ...
          <v-btn
            ...
            fab
            class
            @click="dialogReturnRajapov = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
          ...
          <v-card-text>
            <v-row class="mx-4">
              ...
              <v-col xl="3" lg="3" md="5" sm="5" xs="12" cols="12">
                <!-- From -->
                <v-list>
                  <b style="color: black; font-size: 18px; margin-left: 15px">{{
                    $t("document.confirmers")
                  }}</b>
                  <v-divider></v-divider>
                    ...
                    <v-list-item>
                      <v-list-item-content class="my-0 py-0">
                        ...
                        <v-list-item-subtitle>
                          <b>
                            {{
                              signer.certificate[0].issuerName
                                .split(",")[0]
                                .split("=")[1]
                            }}
                          </b>
          ...
          <v-form name="testform" ref="reactionForm">
            ...
            <v-row>
              <v-col cols="12" class="my-2 py-0">
                ...
                ></select>
                <span v-if="!eimzo_username" style="color: red">{{
                  $t("input.required")
                }}</span>
              ...
              <v-col
                ...
                v-if="
                  [
                    ...
                    500, 506, 507, 508, 510, 511, 512, 513, 514, 515, 521, 527,
                    530, 532, 543, 545, 546, 547, 548, 583, 588, 595,
                  ].includes(document.document_template_id) &&
                  document.status == 0
                "
              >
                <label for>{{ $t("Akt sanasi") }}</label>
                <v-select
                  ...
                  :rules="
                    ...
                    ($store.getters.checkPermission('pomoshnik') &&
                      ...
                      ? [
                          ...
                          (v) =>
                            (!!v && v.length > 10) ||
                            $t('Пожалуйста, введите более 10 символов.'),
                        ]
                      : []
                  "
                ></v-textarea>
                    ...
                    (files) => {
                      ...
                      if (files.length > 0) {
                        ...
                        if (
                          user &&
                          user.employee &&
                          user.employee.employee_staff.length > 0 &&
                          !['3267', '3636'].includes(user.employee.tabel)
                        ) {
                          let userstaffs = user.employee.employee_staff.map(
                            (v) => {
                              return v.staff_id;
                            }
                          );
                          let ds = documentSigners.some(
                            (v) =>
                              v.action_type_id == 16 &&
                              userstaffs.includes(v.staff_id) &&
                              !!v.taken_datetime
                          );
                          let ct = documentSigners.some(
                            (v) =>
                              !(v.status == 1 || v.status == 2) &&
                              v.action_type_id == 11
                          );

                          let children = documentSigners.some(
                            (v) => v.parent_employee_id == user.employee_id
                          );
                          ...
                          if (
                            ...
                            ![11705, 8928, 8916, 11888].includes(
                              user.employee_id
                            ) &&
                            !children
                          ) {
      ...
      <v-card>
        <v-card-title class="grey lighten-3">
          ...
          <v-btn
            ...
            fab
            class
            @click="modalDocumentComment = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
      ...
      <v-card>
        <v-card-title class="grey lighten-3">
          ...
          <v-btn
            ...
            fab
            class
            @click="modalDocumentSubstantiate = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
      ...
      <v-card>
        <v-card-title class="grey lighten-3">
          ...
          <v-btn
            ...
            fab
            class
            @click="modalDocumentPreAgreement = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
      ...
      <v-card>
        <v-card-title class="grey lighten-3">
          ...
          <v-btn
            ...
            fab
            class
            @click="modalDocumentConfirmation = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
      ...
      <v-card>
        <v-card-title class="grey lighten-3">
          ...
          <v-btn
            ...
            fab
            class
            @click="modalEditAttribute = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
              ...
              <v-col cols="9">
                ...
                >
                  <v-col
                    ...
                    class="py-1"
                    v-if="
                      edit_attribute.document_detail_attributes.data_type_id ==
                      1
                    "
                  >
                    <v-text-field
    ...
    <v-dialog v-model="modelAddResolution" persistent width="600">
      <v-card>
        <v-card-title class="grey lighten-3">
          {{ $t("document.resolution") }}
          <v-spacer></v-spacer>
          <v-btn
            ...
            fab
            class
            @click="modelAddResolution = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
          ...
          <v-form ref="resolutionForm">
            <v-row>
              ...
              <v-col cols="6" class="pa-1">
                  ...
                  :items="
                    [305, 357].includes(document.document_template_id)
                      ? document.document_signers.find(
                          (ds) =>
                            user.employee.employee_staff
                              .map((v) => v.staff_id)
                              .includes(ds.staff_id) &&
                            ds.taken_datetime &&
                            (ds.status == 0 || ds.status == 3) &&
                            ds.action_type_id == 16
                        )
                        ? resolutionTypes.filter(
                            (v) => v.is_resolution && v.id != 4
                          )
                        : document.document_signers.find(
                            (ds) =>
                              user.employee.employee_staff
                ...
                >
                  <template v-slot:selection="{ item }">
                    <v-chip color="white" label small class="pa-1 ma-0">
                      <span v-text="item['name_' + $i18n.locale]"></span>
                    </v-chip>
                  </template>
                  <template v-slot:item="{ item }">
      ...
      <v-card>
        <v-card-title class="grey lighten-3">
          ...
          <v-btn
            ...
            fab
            class
            @click="dialogEditDocumentTitle = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    remove(item) {
      console.log(item);
      const index = this.resolution.employees.indexOf(item.id);
      if (index >= 0) this.resolution.employees.splice(index, 1);
    },
    validateEditAttributeForm() {
      if (this.$refs.editAttributeForm) {
        if (
          !this.$refs.editAttributeForm.map((e) => e.validate()).some((v) => !v)
        ) {
          this.modalDocumentReaction = true;
          this.reaction = 1;
        } else Swal.fire(this.$t("edit_attribute_required"));
      } else {
        this.modalDocumentReaction = true;
        this.reaction = 1;
      }
    ...
    getTariffScales() {
      ...
      axios
        .get(
          this.$store.state.backend_url + "api/tariff-scales/get-tariff-scales"
        )
        .then((res) => {
    ...
    meningHujjatimmas(message) {
      this.loading = true;
      axios
        .post(
          this.$store.state.backend_url + "api/documents/mening-hujjatim-emas",
          {
            document_id: this.document.id,
            message: message,
          }
    ...
    blankDownload(blank, document_detail, employee) {
      ...
      form_blank.buyruq_date = this.document.document_date_reg
    ...
    downloadPdfWithComment() {
      axios
        .get(
          this.$store.state.backend_url +
            "api/documents/get-pdf-with-comments/" +
            this.document.id
        )
        .then((res) => {
        ...
        if (result.value) {
          axios
            ...
            .then((res) => {
              // console.log(res);
              this.getList(false);
              this.document.resolution_employee =
                this.document.resolution_employee.filter((v) => {
                  return !(
    ...
    publish() {
      ...
      if (
        [
          488, 489, 490, 491, 492, 493, 494, 495, 496, 497, 498, 499, 500, 506,
          507, 508, 510, 511, 512, 513, 514, 515, 521, 527, 530, 532, 543, 545,
          546, 547, 548, 583, 588, 595,
        ].includes(this.document.document_template_id)
      ) {
        if (this.selectedMonth.length > 0 || this.document.status != 0) {
          axios
            .post(
              this.$store.state.backend_url +
            ...
            .then((res) => {
              ...
              if (res.data.message == "xato") {
                Swal.fire(res.data.message2);
              } else {
          ...
          Swal.fire("Akt sanasini tanlang");
        }
      } else {
        axios
          .post(
    ...
    commentSubstantiate(signer_id) {
      ...
      this.modalDocumentSubstantiate = false;
    },
    comment(signer_id) {
      this.loading = true;
      axios
    ...
    documentReaction(reac, signer_serial_number) {
      ...
      console.log(reac);
      if (reac == 3) {
        this.returnDocument(this.documentComment);
        this.modalDocumentReaction = false;
        this.loading = false;
      } else if (reac == 4) {
        this.meningHujjatimmas(this.documentComment);
        this.modalDocumentReaction = false;
        this.loading = false;
      } else {
        axios
          .post(
            this.$store.state.backend_url + "api/document-signers/reaction",
            {
              document_id: this.document.id,
              description: this.documentComment,
              status: reac,
    ...
    outOnControl() {
      Swal.fire({
        ...
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: this.$t("document.out_on_control"),
      }).then((result) => {
        if (result.value) {
          axios
            .post(
              this.$store.state.backend_url + "api/document/out_on_control",
              {
                document_id: this.document.id,
              }
            )
    ...
    statusReady() {
      // console.log([this.document.reaction_status, this.document.status]);
      if (this.document.document_type_id == 72 && this.document.status == 3) {
        Swal.fire({
          title: this.$t("swal_title"),
          text: this.$t("swal_text"),
          icon: "warning",
          ...
          cancelButtonColor: "#d33",
          confirmButtonText: this.$t("swal_ok"),
          cancelButtonText: this.$t("swal_cancel"),
        }).then((result) => {
          if (result.value) {
            this.loading = true;
            axios
        ...
        .then((res) => {
          ...
          this.document.document_details.map((v) => {
            if (v.document_detail_signer_attributes.length) {
              if (
                v.document_detail_signer_attributes.some(
                  (ddsa) => ddsa.document_detail_attributes.data_type_id == 6
                ) &&
                v.document_detail_signer_attributes.some(
                  (ddsa) => ddsa.document_detail_attributes.table_list_id == 5
                )
              ) {
    ...
    getResolutionEmployees() {
      ...
      axios
        ...
        .then((res) => {
          this.childrenEmployees = this.childrenEmployees.concat(res.data.data);
...
.v-slider__thumb-label {
  transform: translateY(40%) translateY(0px) translateX(-50%) rotate(45deg) !important;
}
