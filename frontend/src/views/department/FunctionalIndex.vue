        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.code="{ item }">
              <span style="white-space: normal; max-width: 100px">
                {{ item.functional_department_code }}
              </span>
            </template>
          </v-data-table>
        </v-col>
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("department.name"), value: "name" },
        { text: this.$t("department.department_code"), value: "code" },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/functional-department", {        
