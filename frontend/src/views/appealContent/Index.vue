  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("appeal_content.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('appeal_content-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('appeal_content-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('appeal_content-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("appeal_content-update") ||
          this.$store.getters.checkPermission("appeal_content-delete")
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/appeal-contents", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("appeal_content-create")) {
        this.dialogHeaderText = this.$t("appeal_content.create");
        this.form = {
          id: Date.now(),
          name: "",
          email: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("appeal_content-update")) {
        this.dialogHeaderText = this.$t("appeal_content.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/appeal-contents/update",
            this.form
          )
          .then((res) => {
            this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("appeal_content-delete")) {
