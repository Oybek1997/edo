  ...
  <div class="fullHeight">
    <v-card class="heightFull" style="border-radius: 10px; border: 1px solid #dce5ef;" elevation="0">
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("organization.index") }}</span>
        <div
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('organizations-update')"
                class="px-0"                
                color="#3FCB5D"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('organizations-delete')"
                class="pl-0 pr-2" 
                color="error"
                style="min-width: 25px;"
                small
    ...
    >
      <v-card>
        <v-card-title style="background-color: #F6F9FB;" primary-title>
          <span class="dialog-head_title">{{ dialogHeaderText }}</span>
          <v-spacer></v-spacer>
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save">
            <v-row  class="ma-0 pa-0 dialog-form">
              <v-col cols="12" md="12" class="pr-2 pl-0 py-0">
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("user.email"),class: "blue-grey lighten-5", value: "email" },
        {
          text: this.$t("actions"),
          value: "actions",
          width: "70",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/organizations", {
    ...
    newItem() {
      this.dialog = true;
      if (this.$store.getters.checkPermission("organizations-create")) {
        this.dialogHeaderText = this.$t("organization.newOrganization");
        this.form = {
          id: Date.now(),
          name_ru: "",
          name_uz_cyril: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("organizations-update")) {
        this.dialogHeaderText = this.$t("organization.updateOrganization");
    ...
    save() {
      axios
        .post(
          this.$store.state.backend_url + "api/organizations/update",
          this.form
        )
        .then((res) => {
          this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("organizations-delete")) {
