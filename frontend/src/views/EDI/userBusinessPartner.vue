    ...
    <div>
      <v-card class="ma-1 pa-1">
        <v-card-title class="pa-1">
          <span>{{ $t("edi.user_business_partners_index") }}</span>
          <v-spacer></v-spacer>
          ...
          <v-card-text>
            <v-form @keyup.native.enter="save" ref="dialogForm">
              <table style="width: 100%" class="mt-2">
                <tr>
                  <td class="pa-1">
                    <label for>{{ $t("edi.userbusinesspartner_user_id") }}</label>
                  </td>
                  <td class="pa-1">
                  <v-autocomplete
                  ...
                  <td class="pa-1">
                    <label for>{{ $t("edi.userbusinesspartner_bp_id") }}</label>
                  </td>
                  <td class="pa-1">
                  <v-autocomplete
                    v-model="form.business_partner_id"
  ...
  export default {
    ...
    computed: {
      ...
      headers() {
        return [
          ...
          {
            text: this.$t("edi.userbusinesspartner_user"),
            value: "user.eimzo_name",
          },
          {
            text: this.$t("edi.userbusinesspartner_bp_id"),
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        axios
          .post(this.$store.state.backend_url + "api/edi/user-business-partner", {
      ...
      getRef() {
        this.loading = true;
        axios
          .post(this.$store.state.backend_url + "api/edi/user-business-partner/get-ref")
          .then((response) => {
            this.users = response.data.users;
            this.business_partners = response.data.business_partners;
            this.loading = false;
          })
          .catch((error) => {
            console.log(error);
      ...
      newItem() {
        this.dialogHeaderText = this.$t("edi.user_business_partner_create");
        this.form = {
          id: Date.now(),
        };
        this.dialog = true;
      ...
      editItem(item) {
        this.dialogHeaderText = this.$t("edi.user_business_partner_edit");
      ...
      save() {
        if (this.$refs.dialogForm.validate())
          axios
            .post(
              this.$store.state.backend_url + "api/edi/user-business-partner/update",
              this.form
            )
            .then((res) => {
              if (this.image) {
            ...
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/edi/user-business-partner/delete/" +
                  item.id
              )
              .then((res) => {
