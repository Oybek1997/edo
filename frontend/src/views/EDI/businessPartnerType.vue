  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        <span>{{ $t("edi.business_partner_type_header") }}</span>
        <v-spacer></v-spacer>
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <table style="width: 100%">
              <tr>
                <td class="pa-1">
                  <label for>{{ $t("edi.business_partner_type_name") }}</label>
                </td>
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("edi.business_partner_type_name"),
          value: "name",
        },
        {
  ...
  methods: {
    ...
    getList() {
      ...
      axios
        .post(
          this.$store.state.backend_url + "api/edi/business-partner-types",
          {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("coefficient-create")) {
        this.dialogHeaderText = this.$t("edi.business_partner_type_create");
        this.form = {
          id: Date.now(),
          name: "",
        };
    ...
    editItem(item) {
      ...
      this.dialogHeaderText = this.$t("edi.business_partner_type_edit");
        ...
        axios
          .post(
            this.$store.state.backend_url +
              "api/edi/business-partner-types/update",
            this.form
          )
          .then((res) => {
            this.getList();
        ...
        Swal.fire({
          ...
          if (result.value) {
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/edi/business-partner-types/delete/" +
                  item.id
              )
              .then((res) => {
