  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        <span>{{ $t("edi.unitmeasure_header") }}</span>
        <v-spacer></v-spacer>
        <v-text-field
          v-model="search"
            ...
            <table style="width: 100%">
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.unitmeasure_name") }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
                    v-model="form.name"
              ...
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.unitmeasure_value") }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
                    v-model="form.value"
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("edi.unitmeasure_name"),
          value: "name",
        },
        {
          text: this.$t("edi.unitmeasure_value"),
          value: "value",
        },
        {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/edi/unit-measures", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("coefficient-create")) {
        this.dialogHeaderText = this.$t("edi.unitmeasure_create");
        this.form = {
          id: Date.now(),
          name: "",
          value: "",
    ...
    editItem(item) {
      ...
      this.dialogHeaderText = this.$t("edi.unitmeasure_edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/edi/unit-measures/update",
            this.form
          )
          .then((res) => {
            this.getList();
