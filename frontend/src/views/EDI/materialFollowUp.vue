  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        <span>{{ $t("edi.material_follow_ups_index") }}</span>
        <v-spacer></v-spacer>
            ...
            <table style="width: 100%">
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{
                    $t("edi.material_follow_ups_description")
                  }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("edi.material_follow_ups_description"),
          value: "description",
        },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/edi/material-follow-ups", {
    ...
    newItem() {
      this.dialogHeaderText = this.$t("edi.material_follow_ups_create");
      this.form = {
        id: Date.now(),
        description: null,
      };
    ...
    editItem(item) {
      this.dialogHeaderText = this.$t("edi.material_follow_ups_edit");
        ...
        axios
          .post(
            this.$store.state.backend_url +
              "api/edi/material-follow-ups/update",
            this.form
          )
          .then((res) => {
            this.getList();
        ...
        Swal.fire({
          ...
          if (result.value) {
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/edi/material-follow-ups/delete/" +
                  item.id
              )
              .then((res) => {
