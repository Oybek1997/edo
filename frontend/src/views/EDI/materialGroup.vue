  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        <span>{{ $t("edi.material_group_header") }}</span>
        <v-spacer></v-spacer>
            ...
            <table style="width: 100%">
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.material_group_name") }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
                    v-model="form.name"
              ...
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.material_group_description") }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("edi.material_group_name"),
          value: "name",
        },
        {
          text: this.$t("edi.material_group_description"),
          value: "description",
        },
        {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/edi/material-groups", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("coefficient-create")) {
        this.dialogHeaderText = this.$t("edi.material_group_create");
        this.form = {
          id: Date.now(),
          name: "",
          description: "",
    ...
    editItem(item) {
      ...
      this.dialogHeaderText = this.$t("edi.material_group_edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/edi/material-groups/update",
            this.form
          )
          .then((res) => {
            this.getList();
            ...
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/edi/material-groups/delete/" +
                  item.id
              )
              .then((res) => {
