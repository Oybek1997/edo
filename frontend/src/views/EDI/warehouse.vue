    ...
    <v-card class="ma-1 pa-1">
      ...
      >
        ...
        <template v-slot:item.type="{ item }">
          {{ item.type == true ? "Physical" : "Virtual" }}
        </template>
            ...
            <table style="width: 100%">
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.warehouse_number") }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
                    v-model="form.warehouse_number"
              ...
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.warehouse_description") }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
              ...
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.warehouse_type") }}</label>
                </td>
                <td class="pa-1">
                  <v-autocomplete
                    v-model="form.type"
                    :items="[
                      { text: 'Physical', value: true },
                      { text: 'Virtual', value: false },
                    ]"
                    :rules="[(v) => !!v || v == 0 || $t('input_required')]"
                    hide-details="auto"
                    dense
                    outlined
                  ></v-autocomplete>
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("edi.warehouse_number"),
          value: "warehouse_number",
        },
        {
          text: this.$t("edi.warehouse_description"),
          value: "description",
        },
        {
          text: this.$t("edi.warehouse_type"),
          value: "type",
        },
        {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/edi/warehouses", {
    ...
    newItem() {
      this.dialogHeaderText = this.$t("edi.warehouse_create");
      this.form = {
        id: Date.now(),
    ...
    editItem(item) {
      this.dialogHeaderText = this.$t("edi.warehouse_edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/edi/warehouses/update",
            this.form
          )
          .then((res) => {
            this.getList();
