  ...
  <div>
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1">
        <span>{{ $t("edi.shipment_type_header") }}</span>
        <v-spacer></v-spacer>
        <v-text-field
          v-model="search"
            ...
            <table style="width: 100%">
              <tr>
                <td class="pa-1 text-right">
                  <label for>{{ $t("edi.shipment_type_name") }}</label>
                </td>
                <td class="pa-1">
                  <v-text-field
                    v-model="form.name"
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("edi.shipment_type_name"),
          value: "name",
        },
        {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/edi/shipment-types", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("coefficient-create")) {
        this.dialogHeaderText = this.$t("edi.shipment_type_create");
        this.form = {
          id: Date.now(),
          name: "",
        };
    ...
    editItem(item) {
      ...
      this.dialogHeaderText = this.$t("edi.shipment_type_edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/edi/shipment-types/update",
            this.form
          )
          .then((res) => {
            this.getList();
