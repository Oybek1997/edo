  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t('Дорилар') }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('medicine-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('medicine-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('medicine-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
    ...
    <v-dialog v-model="dialog" @keydown.esc="dialog = false" persistent max-width="600">
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="hedialogTitleadline">{{ dialogHeaderText }}</span>
        </v-card-title>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t('Дори номи') }}</label>
                <v-text-field
                  v-model="form.name"
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("Дори номи"), value: "name" },
        ...
        v =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("medicine-update") ||
          this.$store.getters.checkPermission("medicine-delete")
      );
    }
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/medpunkt/medicines", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("medicine-create")) {
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("medicine-update")) {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/medpunkt/medicines/update",
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("medicine-delete")) {
