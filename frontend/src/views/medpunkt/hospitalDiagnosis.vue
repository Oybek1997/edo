  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("Шифохона диагностикаси") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('hospital-diagnosis-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('hospital-diagnosis-update')"
                class="px-1"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('hospital-diagnosis-delete')"
                class="px-1"
                color="error"
                style="min-width: 25px"
                small
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("Код диагностикаси") }}</label>
                <v-autocomplete
                  outlined
                  class="pa-0"
                  clearable
                  v-model="form.diagnosis_code_id"
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("Ташхиснинг номи") }}</label>
                <v-text-field
                  v-model="form.name"
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("Ташхиснинг номи"), value: "name", width: 400 },
        { text: this.$t("Код диагностикаси"), value: "diagnosis_code.name", width: 100},
        { text: this.$t("МКБХ"), value: "mkbx", width: 200 },
        { text: this.$t("Фойдаланувчи"), value: "created_by.username", width: 100 },
        {
          text: this.$t("Рўйхатга олинган сана"),
          value: "created_at", width: 100
        },
        {
          text: this.$t("Амал"),
          value: "actions",
        ...
        v =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("hospital-diagnosis-update") ||
          this.$store.getters.checkPermission("hospital-diagnosis-delete")
      );
    }
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/medpunkt/hospital-diagnosis", {
    ...
    newItem() {      
      if (this.$store.getters.checkPermission("hospital-diagnosis-create")) {
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("hospital-diagnosis-update")) {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/medpunkt/hospital-diagnosis/update",
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("hospital-diagnosis-delete")) {
        ...
        Swal.fire({
          ...
          if (result.value) {
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/medpunkt/hospital-diagnosis/delete/" +
                  item.id
              )
              .then(res => {
