  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t('Код диагностикаси') }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('diagnosis-code-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('diagnosis-code-update')"
                class="px-1"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('diagnosis-code-delete')"
                class="px-1"
                color="error"
                style="min-width: 25px"
                small
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("Код диагностикаси"), value: "name" },
        ...
        v =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("diagnosis-code-update") ||
          this.$store.getters.checkPermission("diagnosis-code-delete")
      );
    }
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/medpunkt/diagnosis-codes", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("diagnosis-code-create")) {
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("diagnosis-code-update")) {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/medpunkt/diagnosis-codes/update",
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("diagnosis-code-delete")) {
            ...
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/medpunkt/diagnosis-codes/delete/" +
                  item.id
              )
              .then(res => {
