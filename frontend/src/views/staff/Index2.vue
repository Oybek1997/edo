<template>
  <div class="staff-s">
    <v-card class="ma-1 pa-1">
      <v-card-title class="pa-1 staffs">
        ...
        <template v-slot:body.prepend="{ item }">
          <tr class="prepend_height">
            ...
            <td>
              <v-autocomplete
                clearable
                v-model="filter.expence_type_id"
                :items="
                  expenceTypes.map((v) => ({
            ...
            <td>
              <v-autocomplete
                clearable
                v-model="filter.personal_type_id"
                :items="
                  personalTypes.map((v) => ({
                    text: v['name_' + $i18n.locale],
                    value: v.id,
        ...
        <template v-slot:item="{ item, index }">
          <tr>
            ...
            <td
              ...
              :title="item.department ? item.department['name_' + $i18n.locale] : ''"
            >
            ...
            <td
              ...
              :title="item.range ? item.range['name_' + $i18n.locale] : ''"
            >
              {{ item.range ? item.range.code : "" }}
            </td>
            <td
              style
              :title="item.expence_type ? item.expence_type['name_' + $i18n.locale] : ''"
            >
            ...
            <td
              style
              :title="
                item.personal_type ? item.personal_type['name_' + $i18n.locale] : ''
              "
            >
              {{ item.personal_type ? item.personal_type["name_" + $i18n.locale] : "" }}
            </td>
            <td style="text-align: right">{{ item.rate_count_bp }}</td>
            <td style="text-align: right">{{ item.rate_count }}</td>
            <td
              @click="showItem(item)"
              :style="
                item.employee_staff.filter((v) => v.employee).length > item.rate_count
                  ? 'background-color:#faa; text-align:right; cursor:pointer; color:blue;'
                  : 'text-align:right; cursor:pointer; color:blue;'
              "
              :title="
                item.employee_staff.filter((v) => v.employee).length > item.rate_count
                  ? $t('overload')
            ...
            >
              {{
                item.employee_staff.length > 0
                  ? item.employee_staff.filter((v) => v.employee).length
                  : ""
              }}
            </td>
            <td :style="item.is_active == 0 ? 'color:red;' : 'color:blue;'">
              {{ item.is_active == 0 ? $t("employee.inactive") : $t("employee.active") }}
            </td>
            <td>
              <v-btn
                v-if="$store.getters.checkPermission('shtat-talablar')"
                :color="item.requirements.length ? 'blue' : 'error'"
                small
                icon
                @click="editItemRequirements(item)"
              >
                <v-icon>mdi-information-outline</v-icon>
              </v-btn>
              <v-btn
                v-if="$store.getters.checkPermission('shtat-tahrirlash')"
                color="blue"
                small
                icon
                @click="editItem(item)"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('shtat-fayl')"
                :color="item.files.length ? 'blue' : 'error'"
                small
                icon
                @click="editItemFiles(item)"
              >
                <v-icon>mdi-download</v-icon>
              </v-btn>
              <v-btn
                v-if="
                  $store.getters.checkPermission('shtat-ochirish') &&
                  !item.employee_staff.length
                "
                color="red"
                small
                icon
              ...
              <v-btn
                v-if="$store.getters.checkPermission('shtat-kritik')"
                color="green"
                small
                icon
                :title="$t('critical.index')"
                @click="getStaff(item)"
              >
              ...
              <!-- <v-btn
                ...
                small
                icon
                @click="addShiftCoefficient(item)"
              >
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row>
              <v-col cols="12">
                <label for>{{ $t("staff.department_id") }}</label>
                <v-autocomplete
                  clearable
              ...
              <v-col cols="12">
                <label for>{{ $t("staff.position_id") }}</label>
                <v-autocomplete
                  clearable
              ...
              <v-col cols="4">
                <label for>{{ $t("staff.range_id") }}</label>
                <v-autocomplete
                  clearable
              ...
              <v-col cols="4">
                <label for>{{ $t("staff.rate_count_bp") }}</label>
                <v-text-field
              ...
              <v-col cols="4">
                <label for>{{ $t("staff.rate_count") }}</label>
                <v-text-field
              ...
              <v-col cols="6">
                <label for>{{ $t("staff.personal_type_id") }}</label>
                <v-autocomplete
                  clearable
              ...
              <v-col cols="6">
                <label for>{{ $t("staff.expence_type_id") }}</label>
                <v-autocomplete
                  clearable
              ...
              <v-col cols="3">
                <label for>{{ $t("staff.order_number") }}</label>
                <v-text-field
              ...
              <v-col cols="3">
                <label for>{{ $t("staff.order_date") }}</label>
                <v-text-field
              ...
              <v-col cols="4">
                <label for>{{ $t("staff.branch_id") }}</label>
                <v-autocomplete
                  clearable
              ...
              <v-col cols="4">
                <label for>{{ $t("staff.coefficient") }}</label>
                <v-autocomplete
                  clearable
        ...
        <v-card-text>
          <v-row>
            <v-col cols="6">
              <span
                ><b>{{
                  $t("lavozim_yuriqnomasi") + " yoki " + $t("kasbiy_yuriqnomasi")
                }}</b></span
              >
              <!-- <span><b>{{ $t('lavozim_yuriqnomasi') }}</b></span>                -->
              <v-list-item-content
                v-for="(i, ind) in relation_document.document_staffs"
                :key="ind"
              >
                <v-list-item-title>
                  <b>{{ "ID" }} : </b>
            ...
            <v-col v-if="relation_document.department" cols="6">
              <span
                ><b>{{ $t("tarkibiy_tuzilma") }}</b></span
              >
              <v-list-item-content
                v-for="(i, ind) in relation_document.department.documents"
                :key="ind"
              >
                <v-list-item-title>
                  <b>{{ "ID" }} : </b>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            ...
            <v-simple-table>
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="(item, index) in staffTmp.requirements" :key="index">
                    <td>{{ index + 1 }}</td>
    ...
    <v-dialog v-model="showItemDialog" persistent max-width="800">
      <v-card>
        <v-card-title class="headline">
          ...
          <v-spacer></v-spacer>
          <v-btn color="red" outlined x-small fab class @click="showItemDialog = false">
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
        <v-card-text>
          ...
          <v-simple-table>
            <template v-slot:default>
              ...
              <tbody>
                <tr
                  v-for="(employee, index) in employeess.filter((v) => v.employee)"
                  :key="index"
                >
                  <td>{{ index + 1 }}</td>
                  <td>
    ...
    <v-dialog
      v-model="dialogShiftCoefficient"
      @keydown.esc="dialogShiftCoefficient = false"
      persistent
      max-width="800px"
    >
      <v-card>
        <v-card-title>
          ...
          <v-btn
            ...
            fab
            class
            @click="dialogShiftCoefficient = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row>
              <v-col cols="12">
                <label for>{{ $t("department.coefficients") }}</label>
                <v-autocomplete
                  chips
                  clearable
                  v-model="form.multiplecoefficients"
                  :items="coefficients"
                  :rules="[(v) => !!v || $t('input.required')]"
                  item-value="id"
                  :item-text="'description'"
              ...
              <v-col cols="12">
                <label for>{{ $t("department.shift") }}</label>
                <v-autocomplete
                  chips
                  clearable
                  v-model="form.multiplecoefficients"
...
export default {
  ...
  computed: {
    ...
    departmentList() {
      return this.departments.map((value) => {
        ...
        v.name_ru = v.department_code ? v.department_code + " " + v.name_ru : "";
        return v;
      });
    },
  },
  methods: {
      ...
      axios
        ...
        .then((response) => {
          response.data.data.map((element, index) => {
            if (element.employee_staff) {
              let language = this.$i18n.locale == "ru" ? "uz_cyril" : this.$i18n.locale;
              let employee = "";
              let tabel = "";
    ...
    addFiles() {
      ...
      axios
        .post(
          this.$store.state.backend_url + "api/staffs/update-files/" + this.staffTmp.id,
          this.formData,
          {
            headers: {
    ...
    addRequirement() {
      let requirement = this.requirements.find((v) => v.id == this.selectRequirement);
    ...
    addShiftCoefficient(item) {
      if (this.$store.getters.checkPermission("staff-update")) {
        this.dialogHeaderText = this.$t("staff.editStaff");
        this.form = Object.assign({}, item);
        this.dialogShiftCoefficient = true;
        this.editMode = true;
        if (this.$refs.dialogForm) this.$refs.dialogForm.resetValidation();
      }
    },
    ...
    getStaff(item) {
      ...
      axios
        .get(this.$store.state.backend_url + "api/staff-criticals/show/" + item.id)
        .then((response) => {
          if (response.data) {
    ...
    saveRequirements() {
      axios
        .post(this.$store.state.backend_url + "api/staffs/update-requitements", {
          staff: this.staffTmp,
        })
        .then((res) => {
      ...
      Swal.fire({
        ...
        if (result.value) {
          if (this.$store.getters.checkPermission("staff-delete")) {
            ...
            axios
              .get(this.$store.state.backend_url + "api/staffs/deactivate/" + item.id)
              .then((res) => {
