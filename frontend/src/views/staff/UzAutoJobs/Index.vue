  ...
  <div class="fullHeight">
    ...
    >
      ...
      >
        ...
        <template v-slot:item.status="{ item, index }" style>
          <!-- {{ item.choice_status[item.choice_status.length - 1].status }} -->
          <span
            v-if="item.choice_status[item.choice_status.length - 1].status == 201"
            class="tdClass1 text-center"
            style="background: orange"
          >
            {{
              itemStatus.find(
                (v) => v.id == item.choice_status[item.choice_status.length - 1].status
              ).text
            }}
          </span>
          <span
            v-if="item.choice_status[item.choice_status.length - 1].status == 202"
            class="tdClass1 text-center"
          ...
          <span
            v-if="item.choice_status[item.choice_status.length - 1].status == 200"
            class="tdClass1 text-center"
        ...
        <template v-slot:item.id="{ item, index }" style>
          ...
          <span
            ...
            :style="item.choice_status[item.choice_status.length - 1].status == 200?'cursor: pointer':''"
          >
          {{ String(item.id).padStart(6, '0') }}
          </span>
        </template>
...
export default {
  ...
  methods: {
    ...
    getSelection(item)
    {
      this.$router.push("/staffs/employees/candidates/" +item.id);
    },
    getList() {
      ...
      axios
        ...
        .then((response) => {
          ...
          // this.from = response.data.choice.from;
          // this.server_items_length = response.data.choice.total;
          this.loading = false;
        })
        .catch((error) => {
          console.log(error);
