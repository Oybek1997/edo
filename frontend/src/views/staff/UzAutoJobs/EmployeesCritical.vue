  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("Vakansiyalar bilan ishlash") }}</span>
      ...
      >
        ...
        <template v-slot:item.itemSave="{ item }" style>
          <v-tooltip bottom>
          ...
          text
              ...
              v-on="on"
              dark
              @click="saveKritik(item)">
                  <v-icon color="#6ac82d">mdi-content-save</v-icon>
                </v-btn>
              </template>
                <span>{{ $t("Ўзгаришларни сақлаш") }}</span>
                </v-tooltip>
        </template>
        ...
        <template v-slot:item.vakant="{ item }" style>
               <p style="color:#6ac82d" >vakant</p>     
        </template>
        <template v-slot:item.comment="{ item }" style>
               <p style="color:red" >{{item.comment?'Rad qilindi: '+item.comment:''}}
                </p>     
        </template>
        <template v-slot:item.status_vacancy="{ item }" style>
              <template v-for="itemVacancy in item.employee">                    
                    
                <v-checkbox
                      v-model="itemVacancy.kritik"
                      hide-details
                      :disabled="itemVacancy.sending||itemVacancy.band||itemVacancy.block"
                    ></v-checkbox>                   
              </template>
            </template>
        <template v-slot:item.status_sending="{ item }" style>
              <template v-for="itemVacancy in item.employee">                    
                    <v-checkbox
                      v-model="itemVacancy.sending"
                      hide-details
                      disabled
                    ></v-checkbox>                   
              </template>
            </template>
        <template v-slot:item.status_band="{ item }" style>
              <template v-for="itemVacancy in item.employee">                    
                    <v-checkbox
                      v-model="itemVacancy.band"
                      hide-details
                      disabled
                    ></v-checkbox>                   
              </template>
              </template>
        <template v-slot:item.status_block="{ item }" style>
              <template v-for="itemVacancy in item.employee">                    
                    <v-checkbox
                      v-model="itemVacancy.block"
                      hide-details
                    ></v-checkbox>                   
              </template>
            </template>
...
export default {
    ...
    headers() {
      return [
        ...
        },       
        {
          text: this.$t("FISH"),
          value: "vakant",
          class: "blue-grey lighten-5 black--text pa-2",
          sortable: false,
  ...
  methods: {
    ...
    saveKritik(item)
    {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/employe-staff/save-critical", {
          data: item,
        })
        .then((response) => {
          this.getList() ;
    ...
    getList() {
      ...
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/employe-staff/vacancies-critical", {
