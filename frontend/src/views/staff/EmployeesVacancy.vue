  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("staff.vacancy") }}</span>
        <div class="headerSearch d-flex align-center">
          ...
          disabled
            ...
            @click="getList"
          >
            <v-icon color="#00B950" left>mdi-magnify</v-icon>{{ $t("download") }}
          </v-btn>
          <v-btn
            class="filterBtn px-2"
            style="background: #fff; height: 34px"
        ...
        <template v-slot:item.sendJobs="{ item, index }" style>
          ...
          v-if="((item.rate_count_bp - item.employeeCount)-item.rate_count_sv>0)">
            <template v-slot:activator="{ on, attrs }">
              <v-btn dark v-bind="attrs" v-on="on" @click="sendJobs(item)" icon>
        ...
        <template v-slot:item.minReq="{ item }" style>
          <template v-if="item.staf_min_req && item.staf_min_req.length">
            <tr v-for="(itema, index) in item.staf_min_req" :key="index">
              <td rowspan="4" class="text-left" v-if="index === 0">
                <v-btn class="ma-1" color="#6ac82d" x-small dark @click="changeRec(item)">
          ...
          <v-card-text class="pt-0">
            ...
            <!-- <v-col cols="12" class="pa-0">
              <v-input
                :messages="
                  'FC code: ' +
                  (sendItem.department && sendItem.department.functional_department
                    ? sendItem.department.functional_department.functional_department_code
                    : '')
                "
              ...
              >
                {{
                  sendItem.department
                    ? sendItem.department.functional_department["name_" + $i18n.locale]
                    : ""
                }}
              </v-input>
            </v-col> -->
            ...
            <span class="hTitle pa-0">{{ $t("Мавжуд бўш ўринлар:") }}</span>
            <template>
              <v-simple-table>
                <template v-slot:default>
                  <thead>
                    <tr>
                      <th class="text-center">Режа вакант</th>
                      <th class="text-center">Мавжуд ходим</th>
                      <th class="text-center">Вакант</th>
                      <th class="text-center">Юборилган вакант</th>
                      <th class="text-center">Мавжуд вакант</th>
                      <th class="text-center">Юбориладиган</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td class="text-center">{{ sendItem.rate_count_bp }}</td>
                      <td class="text-center">
                        {{ sendItem.employeeCount ? sendItem.employeeCount : 0 }}
                      </td>
                      <td class="text-center">
                        {{
                          sendItem.rate_count_bp -
                          (sendItem.employeeCount ? sendItem.employeeCount : 0)
                        }}
                      </td>
                      <td class="text-center">{{ sendItem.rate_count_sv }}</td>
                      <td class="text-center">
                        {{
                          sendItem.rate_count_bp -
                          (sendItem.employeeCount ? sendItem.employeeCount : 0) -
                          sendItem.rate_count_sv
                        }}
                      </td>
                      <td style="width: 50px">
                        <v-text-field
                          v-model="filter.sendCount"
                          hide-details
                          dense
                          type="number"
                          :rules="
          ...
          <v-card class="mt-1 text-end" outlined elevation="0" style="border: none">
            <v-btn
              v-if="
                ...
                sendItem.rate_count_bp -
                  (sendItem.employeeCount ? sendItem.employeeCount : 0) -
                  sendItem.rate_count_sv)&&filter.sendCount>0
              "
    ...
    <v-dialog v-model="dialogRec" max-width="800px" persistent>
      <v-card class="ma-0 pa-3" style="border-radius: 1px">
        <span class="dialog-head_title">{{ $t("requirement.index") }}</span>
        <v-divider class="mt-1 mb-3" style="border-color: #dce5ef"></v-divider>
        <v-card-text class="pl-0">
          <v-card-text class="pt-0">        
            <v-row>
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("positionType.code"),
          value: "position_code",
          class: "blue-grey lighten-5 lign-center",
        },
        {
          text: this.$t("positionType.index"),
          value: "position_name",
          class: "blue-grey lighten-5 lign-center",
        },
        {
        ...
        {
          text: this.$t("requirement.index"),
          value: "minReq",
          class: "blue-grey lighten-5 lign-center",
        },
        {
          text: this.$t("BPV"),
          value: "b_rate_count_vacant",
          class: "blue-grey lighten-5 center",
        },
        {
          text: this.$t("staff.sendedvacancy"),
          value: "rate_count_sv",
          class: "blue-grey lighten-5 center",
        },
      ];
