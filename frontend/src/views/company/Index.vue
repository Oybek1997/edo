  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("company.index") }}</span>
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('company-update')"
                class="px-0"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('company-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/companies")
        .then((response) => {
