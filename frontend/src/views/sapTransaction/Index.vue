  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("sap_transaction.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('sap-transactions-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('sap-transactions-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('sap-transactions-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
          ...
          <v-form @keyup.native.enter="save">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("sap_transaction.index") }}</label>
                <v-text-field
                  v-model="form.extended_name"
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("name"), value: "extended_name" },
        {
          text: this.$t("actions"),
          value: "actions",
          width: 120,
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("sap-transactions-update") ||
          this.$store.getters.checkPermission("sap-transactions-delete")
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/sap-transactions", {
    ...
    newItem() {
      this.dialog = true;
      if (this.$store.getters.checkPermission("sap-transactions-create")) {
        this.dialogHeaderText = this.$t("sap_transaction.add");
        this.form = {
          id: Date.now(),
          extended_name: "",
        };
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("sap-transactions-update")) {
    ...
    save() {
      axios
        .post(
          this.$store.state.backend_url + "api/sap-transactions/update",
          this.form
        )
        .then((res) => {
          this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("sap-transactions-delete")) {
