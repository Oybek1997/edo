  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("requirement.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('requirement-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('requirement-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('requirement-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("requirementType.index") }}</label>
                <v-autocomplete
                  clearable
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("requirement.requirement_type_id"),
          value: "requirement_type_id",
        },
        {
          text: this.$t("actions"),
          value: "actions",
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("requirement-update") ||
          this.$store.getters.checkPermission("requirement-delete")
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/requirements", {
          pagination: this.dataTableOptions,
          search: this.search,
        })
        .then((response) => {
          this.items = response.data.requirements.data;
          this.from = response.data.requirements.from;
          this.server_items_length = response.data.requirements.total;
          this.requirement_types = response.data.requirement_types.map((v) => {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("requirement-create")) {
        this.dialogHeaderText = this.$t("requirement.newrequirement");
        this.form = {
          id: Date.now(),
          name: "",
          email: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("requirement-update")) {
        this.dialogHeaderText = this.$t("requirement.updaterequirement");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/requirements/update",
            this.form
          )
          .then((res) => {
            this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("requirement-delete")) {
