  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("hr_state_award.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('hr-state-award-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('hr-state-award-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('hr-state-award-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("hr-state-award-update") ||
          this.$store.getters.checkPermission("hr-state-award-delete")
      );
    },
  },
  methods: {
    ...
    newItem() {
      this.dialog = true;
      if (this.$store.getters.checkPermission("hr-state-award-create")) {
        this.dialogHeaderText = this.$t("hr_state_award.add");
        this.form = {
          id: Date.now(),
          name_ru: "",
          name_uz_cyril: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("hr-state-award-update")) {
    ...
    save() {
      axios
        .post(
          this.$store.state.backend_url + "api/hr-state-awards/update",
          this.form
        )
        .then((res) => {
          this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("hr-state-award-delete")) {
