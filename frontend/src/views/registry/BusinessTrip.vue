<template>
  <div class="fullHeight">
        ...
        <v-col class="ma-0 pa-0" xs="12">
          <v-simple-table 
            ...
            <template v-slot:default>
              <thead>
                <tr>
                  ...
                  <th
                    ...
                    v-if="
                      items.filter(v => v.document_template_id == 427)[0] &&
              ...
              <tbody v-for="(item, key) in items.filter(v => v.document_template_id == 9)" :key="key">
                <tr
                ...
                >
                  ...
                  >
                    {{
                      item.employee.main_staff[0] && item.employee.main_staff[0].department["name_" + $i18n.locale]
                    }}
                  </td>
                  ...
                  <td
                    @click="pdfViewInDialog(item)"
                    v-if="
                      items.filter(v => v.document_template_id == 9)[0] &&
                      items.filter(v => v.document_template_id == 9)[0].document_details[0].document_detail_employees
                        .length > 0
                    "
                  >
                    <span
                  ...
                  <template
                    v-for="(
                      attr, attr_key
                    ) in items.filter(v => v.document_template_id == 9)[0].document_template.document_detail_templates[0].document_detail_attributes.filter(
                  ...
                  <td
                    ...
                    v-if="detail_key == 0"
                  >
                    <v-icon size="18" class="ml-3" color="red" @click="deleteItem(item)"
                      >mdi-trash-can-outline</v-icon
                    >
                  </td>
                </tr>
              </tbody>
              <tbody v-for="(item, key) in items.filter(v => v.document_template_id == 427)" :key="key+'abc'">
                <tr
                  style="background-color: #ffffff; cursor: pointer"
                  v-for="(detail, detail_key) in item.document_details"
                  :key="detail_key"
                >
                  ...
                  <td
                    ...
                    v-if="
                      items.filter(v => v.document_template_id == 427)[0] &&
                      items.filter(v => v.document_template_id == 427)[0].document_details[0].document_detail_employees
                        .length > 0
                    "
                  >
                    <span
                  ...
                  <template
                    v-for="(
                      attr, attr_key
                    ) in items.filter(v => v.document_template_id == 427)[0].document_template.document_detail_templates[0].document_detail_attributes.filter(
                      (v) => v.is_registry_show == 1 && v.is_active == 1
                    )"
                  >
                    <template v-if="attr.table_list_id != null">
    ...
    <v-dialog v-model="modalDocumentReaction" persistent width="500">
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="dialogTitle">{{ $t("document.reaction") }}</span>
        </v-card-title>
        <v-divider color="#DCE5EF" class="my-1"></v-divider>
        <v-card-text>
          <v-form name="testform">
            ...
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3" v-if="sign_type">
                Выберите ключ
                <br />
                <select
                  name="key"
              ...
              <v-col cols="6" style="min-width: 100px" class="pa-0 mb-3 text-right">
                <v-img
                  v-if="sign_type"
                  :src="'img/ldap.jpg'"
                  width="70"
...
export default {
  ...
  methods: {
    makeDocument() {
      ...
      axios
        .post(
          this.$store.state.backend_url + "api/business-trip-registry-create",
          {
            ids: ids,
    ...
    getList() {
      ...
      axios
        .get(
          this.$store.state.backend_url + "api/business-trip-registry/" + locale
        )
        .then((response) => {
