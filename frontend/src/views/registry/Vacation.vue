  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("reysters") }}:
          {{
            ...
            <template v-slot:default>
              <thead>
                <tr>
                  ...
                  <th style="background-color: #f0f0f5">Tatil turi</th>
                  <th style="background-color: #f0f0f5">Boshlanish sanasi</th>
                  <th style="background-color: #f0f0f5">Necha kunga</th>
                  <th style="background-color: #f0f0f5">Moddiy yordam</th>
                  <th style="background-color: #f0f0f5">Qaysi yillar uchun</th>
                  <th style="background-color: #f0f0f5">Ish staji</th>
              ...
              <tbody v-for="(item, key) in items" :key="key">
                ...
                >
                  ...
                  >
                    {{
                      item.employee.main_staff &&
                      item.employee.main_staff.length > 0
                        ? item.employee.main_staff[0].department[
                            "name_" + $i18n.locale
                          ]
                        : item.employee.staff
                        ? item.employee.staff[0].department["name_" + $i18n.locale]
                        : ""
                    }}
                  </td>
                  ...
                  >
                    ...
                    >
                      {{
                        detail_employee.employee.lastname_uz_latin +
                        " " +
                        detail_employee.employee.firstname_uz_latin
                      }}
                      <br />
                    </span>
                  </td>
                  <td>
                    {{
                      item.document_template_id == 12 ? "T" : (item.document_template_id == 474 ? "D" : "S")
                    }}
                  </td>
                  <td>
                    {{
                      detail.document_detail_attribute_values.some(v => [159,1519,2569].includes(v.d_d_attribute_id))
                      ? detail.document_detail_attribute_values.find(v => [159,1519,2569].includes(v.d_d_attribute_id)).attribute_value
                      : ""
                    }}
                  </td>
                  <td>
                    {{
                      detail.document_detail_attribute_values.some(v => [160,1520,2570].includes(v.d_d_attribute_id))
                      ? detail.document_detail_attribute_values.find(v => [160,1520,2570].includes(v.d_d_attribute_id)).attribute_value
                      : ""
                    }}
                  </td>
                  <td>
                    {{
                      detail.document_detail_attribute_values.some(v => [325].includes(v.d_d_attribute_id))
                      ? 1
                      : ""
                    }}
                  </td>
                  <td>
                    {{
                      detail.document_detail_attribute_values.some(v => [421,1521,2572].includes(v.d_d_attribute_id))
                      ? detail.document_detail_attribute_values.find(v => [421,1521,2572].includes(v.d_d_attribute_id)).table_value
                      : ""
                    }}
                  </td>
                  <td>
                    {{
                      detail.document_detail_attribute_values.some(
                        (v) => v.d_d_attribute_id == 2571
                      )
    ...
    >
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="dialogTitle">{{ $t("company.dialogHeaderText") }}</span>
        </v-card-title>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("user.name") }}</label>
                <v-text-field
                  v-model="form.name"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = this.loading + 1;
      axios
        .get(this.$store.state.backend_url + "api/vacation-registry/" + locale)
        .then((response) => {
              ...
              dd.document_detail_attribute_values =
                dd.document_detail_attribute_values.map((ddav) => {
                  if (ddav.document_detail_attributes.table_list)
                    if (ddav.document_detail_attributes.table_list_id == 14)
