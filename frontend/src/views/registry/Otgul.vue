          ...
          <template v-slot:default>
            <thead>
              <tr>
                ...
                <th style="background-color: #f0f0f5">Chiqish sanasi</th>
            ...
            <tbody v-for="(item, key) in items" :key="key">
              ...
              >
                ...
                <td>
                  {{
                    detail.document_detail_attribute_values.some(v => [2595].includes(v.d_d_attribute_id))
                    ? detail.document_detail_attribute_values.find(v => [2595].includes(v.d_d_attribute_id)).attribute_value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_attribute_values.some(v => [2597].includes(v.d_d_attribute_id))
                    ? detail.document_detail_attribute_values.find(v => [2597].includes(v.d_d_attribute_id)).attribute_value
                    : ""
                  }}
                </td>
                <!-- <td>
  ...
  export default {
    ...
    methods: {
      makeDocument() {
        ...
        axios
          .post(this.$store.state.backend_url + "api/otgul-registry-create", {
            ids: ids,
      ...
      getList() {
        ...
        this.loading = this.loading + 1;
        axios
          .get(this.$store.state.backend_url + "api/otgul-registry/" + locale)
          .then((response) => {
