<template>
    <div>
      <v-card class="ma-1 pa-1">
        <v-card-title class="pa-1">
          <span
            >{{ $t("LSP REYESTR") }}:
            {{
              items[0] ? items[0].document_template["name_" + $i18n.locale] : ""
            }}</span
          >
          <v-spacer></v-spacer>
          <!-- <v-text-field
            class="mr-2"
            v-model="from_date"
            label="Sanadan"
            type="date"
            outlined
            dense
            style="max-width: 15%"
            clearable
            single-line
          hide-details
          ></v-text-field>
          <v-text-field
            class="mr-2"
            v-model="to_date"
            label="Sanagacha"
            type="date"
            outlined
            dense
            style="max-width: 15%"
            clearable
            single-line
          hide-details
          ></v-text-field>
        <v-btn depressed dense color="success" @click="getList()">
          Izlash
        </v-btn> -->
          <v-btn
            small
            outlined
            color="success"
            class="mx-2"
            @click="makeDocument"
            v-if="items.filter((v) => !!v.selected).length > 0"
            >{{ $t("create") }}
          </v-btn>
        </v-card-title>
        <v-simple-table
          dense
          fixed-header
          :height="screenHeight"
          class="mainTable"
        >
          <template v-slot:default>
            <thead>
              <tr>
                <th rowspan="2" style="background-color: #f0f0f5">#</th>
                <th rowspan="2" style="background-color: #f0f0f5">
                  {{ $t("document.document_number") }}
                </th>
                <th rowspan="2" style="background-color: #f0f0f5">
                  {{ $t("document.document_date") }}
                </th>

  
                <th colspan="7" style="background-color: #f0f0f5; text-align: center; ">Yetkazib beruvchi</th>
                <th rowspan="2" style="background-color: #f0f0f5">To'lov maqsadi</th>
                <th rowspan="2" style="background-color: #f0f0f5">Mas'ul boshqarma</th>
                <th rowspan="2" style="background-color: #f0f0f5">AS</th>
                <th rowspan="2" style="background-color: #f0f0f5">PAS</th>
                <th rowspan="2" style="background-color: #f0f0f5">Pul birligi</th>
                <th rowspan="2" style="background-color: #f0f0f5">Shartnoma (nomer-sana)</th>
                <th rowspan="2" style="background-color: #f0f0f5">Debet</th>
                <th rowspan="2" style="background-color: #f0f0f5">Kredit</th>
                <th rowspan="2" style="background-color: #f0f0f5">To'lov amalaga oshiriladigan oxirgi sana</th>
                <th rowspan="2" style="background-color: #f0f0f5">Shartnoma summasi</th>
                <th rowspan="2" style="background-color: #f0f0f5">Shartnoma bo'yicha oldindan to'lov</th>
                <th rowspan="2" style="background-color: #f0f0f5">Shartnoma bo'yicha joriy to'lov</th>
                <th rowspan="2" style="background-color: #f0f0f5">Qoldiq</th>
                <th rowspan="2" style="background-color: #f0f0f5">To'lov reyestriga kiritilgan sana</th>
                <th rowspan="2" style="background-color: #f0f0f5">To'lov topshiriq nomasi raqami va sanasi</th>
                <th rowspan="2" style="background-color: #f0f0f5">Status</th>
                <!-- <th style="background-color: #f0f0f5">Kredit</th> -->
                <th rowspan="2" style="background-color: #f0f0f5" width="40">
                  <v-checkbox
                    class="ml-3"
                    v-model="select_all"
                    @click="selectAll()"
                    :indeterminate="indeterminate"
                  ></v-checkbox>
                </th>
                <th rowspan="2" style="background-color: #f0f0f5" width="40"></th>
              </tr>
              <tr>
                <th style="background-color: #f0f0f5">STIR</th>
                <th style="background-color: #f0f0f5">Kontragent nomi</th>
                <th style="background-color: #f0f0f5">Manzil</th>
                <th style="background-color: #f0f0f5">Bank nomi</th>
                <th style="background-color: #f0f0f5">Hisob raqam</th>
                <th style="background-color: #f0f0f5">SWIFT kodi</th>
                <th style="background-color: #f0f0f5">MFO</th>
              </tr>
            </thead>
            <tbody v-for="(item, key) in items" :key="key">
              <tr
                style="background-color: #ffffff; cursor: pointer"
                v-for="(detail, detail_key) in item.document_details"
                :key="detail_key"
              >
                <td
                  :rowspan="detail_key == 0 ? item.document_details.length : 1"
                  v-if="detail_key == 0"
                  @click="pdfViewInDialog(item)"
                >
                  {{ key + 1 }}
                </td>
                <td
                  @click="pdfViewInDialog(item)"
                  :rowspan="detail_key == 0 ? item.document_details.length : 1"
                  v-if="detail_key == 0"
                >
                  {{ item.document_number }}
                </td>
                <td
                  @click="pdfViewInDialog(item)"
                  :rowspan="detail_key == 0 ? item.document_details.length : 1"
                  v-if="detail_key == 0"
                >
                  {{ item.document_date.substring(0, 10) }}
                </td>
                
                <td>
                  {{
                    detail.document_detail_contents.some(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id) && v.attribute_name=='inn').value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id) && v.attribute_name=='name').value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id) && v.attribute_name=='adress').value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id) && v.attribute_name=='bank_name').value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id) && v.attribute_name=='account').value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id) && v.attribute_name=='swift_code').value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [366,381,605,806,2454,2471,2535,2552].includes(v.d_d_attribute_id) && v.attribute_name=='mfo').value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [367,382,606, 629, 807, 1394, 2457, 2474, 2538, 2555].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [367,382,606, 629, 807, 1394, 2457, 2474, 2538, 2555].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [1467,1468,1476,1477,1478,1482,2463,2480,2544,2561].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [1467,1468,1476,1477,1478,1482,2463,2480,2544,2561].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [2602,2604,2606,2608,2610,2612,2614,2616,2618,2620].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [2602,2604,2606,2608,2610,2612,2614,2616,2618,2620].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [2603,2605,2607,2609,2611,2613,2615,2617,2619,2621].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [2603,2605,2607,2609,2611,2613,2615,2617,2619,2621].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [1381,1382,1388,1389,1390,1409,2456,2473,2537,2554].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [1381,1382,1388,1389,1390,1409,2456,2473,2537,2554].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [371,386,610,633,811,1398,2460,2477,2541,2558].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [371,386,610,633,811,1398,2460,2477,2541,2558].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_signer_attributes.some(v => [379,394,618,641,823,1407,2469,2486,2550,2567].includes(v.d_d_attribute_id))
                    ? detail.document_detail_signer_attributes.find(v => [379,394,618,641,823,1407,2469,2486,2550,2567].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_signer_attributes.some(v => [380,395,619,642,824,1408,2470,2487,2551,2568].includes(v.d_d_attribute_id))
                    ? detail.document_detail_signer_attributes.find(v => [380,395,619,642,824,1408,2470,2487,2551,2568].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [325].includes(v.d_d_attribute_id))
                    ? 1
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [374,389,613,636,814,1402,2464,2481,2545,2562].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [374,389,613,636,814,1402,2464,2481,2545,2562].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [375,390,614,637,815,1403,2465,2482,2546,2563].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [375,390,614,637,815,1403,2465,2482,2546,2563].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [376,391,615,638,816,1404,2466,2483,2547,2564].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [376,391,615,638,816,1404,2466,2483,2547,2564].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td>
                  {{
                    detail.document_detail_contents.some(v => [378,393,617,640,818,1406,2468,2485,2549,2566].includes(v.d_d_attribute_id))
                    ? detail.document_detail_contents.find(v => [378,393,617,640,818,1406,2468,2485,2549,2566].includes(v.d_d_attribute_id)).value
                    : ""
                  }}
                </td>
                <td></td>
                <td></td>
                <td></td>
                <td
                  width="40"
                  :rowspan="detail_key == 0 ? item.document_details.length : 1"
                  v-if="detail_key == 0"
                >
                  <v-checkbox class="ml-3" v-model="item.selected"></v-checkbox>
                </td>
                <td
                  width="40"
                  :rowspan="detail_key == 0 ? item.document_details.length : 1"
                  v-if="detail_key == 0"
                >
                  <v-icon class="ml-3" color="red" @click="deleteItem(item)"
                    >mdi-delete</v-icon
                  >
                </td>
              </tr>
            </tbody>
          </template>
        </v-simple-table>
      </v-card>
  
      <v-dialog v-model="pdfViewDialog" fullscreen>
        <v-card>
          <v-card-title primary-title>
            <v-spacer></v-spacer
            ><v-btn color="red" outlined @click="pdfViewDialog = false">{{
              $t("close")
            }}</v-btn>
          </v-card-title>
          <v-card-text class="d-flex justify-center">
            <iframe
              v-if="pdfViewDialog"
              :width="screenWidth - 200"
              :height="screenHeight - 20"
              :src="'data:application/pdf;base64,' + selected_document.pdf"
            ></iframe>
          </v-card-text>
          <v-divider></v-divider>
          <v-card-actions>
            <v-spacer></v-spacer
            ><v-btn
              color="red"
              outlined
              @click="pdfViewDialog = false"
              class="mr-4"
              >{{ $t("close") }}</v-btn
            >
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-dialog
        v-model="dialog"
        @keydown.esc="dialog = false"
        persistent
        max-width="800"
      >
        <v-card>
          <v-card-title class="headline grey lighten-2" primary-title>
            <span class="headline">{{ $t("company.dialogHeaderText") }}</span>
            <v-spacer></v-spacer>
            <v-btn color="red" outlined x-small fab class @click="dialog = false">
              <v-icon>mdi-close</v-icon>
            </v-btn>
          </v-card-title>
          <v-card-text>
            <v-form @keyup.native.enter="save" ref="dialogForm">
              <v-row>
                <v-col cols="12">
                  <label for>{{ $t("user.name") }}</label>
                  <v-text-field
                    v-model="form.name"
                    :rules="[(v) => !!v || $t('input_required')]"
                    hide-details="auto"
                    dense
                    outlined
                  ></v-text-field>
                </v-col>
              </v-row>
            </v-form>
            <small color="red">{{ $t("input_required") }}</small>
          </v-card-text>
          <v-divider></v-divider>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="save">{{ $t("save") }}</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-dialog v-model="modalDocumentReaction" persistent width="500">
        <v-card>
          <v-card-title class="grey lighten-3">
            {{ $t("document.reaction") }}
            <v-spacer></v-spacer>
            <v-btn
              color="red"
              outlined
              x-small
              fab
              class
              @click="modalDocumentReaction = false"
            >
              <v-icon>mdi-close</v-icon>
            </v-btn>
          </v-card-title>
          <v-card-text>
            <v-form name="testform">
              <v-text-field name="pkcs7" v-show="false"></v-text-field>
              <v-row>
                <v-col cols="12" class="my-2 py-0" v-if="sign_type">
                  Выберите ключ
                  <br />
                  <select
                    name="key"
                    @change="cbChanged(this)"
                    style="border: 1px solid black"
                    class="pa-2 v-input__control"
                  ></select>
                  <br />
                  <label v-show="false" id="keyId"></label>
                </v-col>
                <v-col cols="12">
                  <v-textarea
                    :label="$t('document.comment')"
                    v-model="documentComment"
                    rows="1"
                    row-height="15"
                    dense
                    outlined
                    auto-grow
                    hide-details="auto"
                  ></v-textarea>
                </v-col>
                <v-col cols="6" style="min-width: 100px" class="text-right">
                  <v-img
                    v-if="sign_type"
                    :src="'img/ldap.jpg'"
                    width="70"
                    class="mt-2 ml-auto"
                    contain
                    style="cursor: pointer"
                    @click="
                      sign_type = !sign_type;
                      AppLoad();
                    "
                  ></v-img>
                  <v-img
                    v-if="!sign_type"
                    :src="'img/imzo.png'"
                    width="100"
                    class="mt-2 ml-auto"
                    contain
                    style="cursor: pointer"
                    @click="
                      sign_type = !sign_type;
                      AppLoad();
                    "
                  ></v-img>
                </v-col>
                <v-col cols="6" style="min-width: 100px" class="text-right">
                  <v-btn
                    v-if="reaction == 1"
                    outlined
                    width="200px"
                    color="success"
                    class="mx-2"
                    @click="actionSelectedDocuments(1)"
                  >
                    <span style="white-space: normal; width: 180px">{{
                      $t("document.accept")
                    }}</span>
                  </v-btn>
                  <v-btn
                    v-else
                    color="error"
                    outlined
                    width="200px"
                    class="mx-2"
                    @click="actionSelectedDocuments(2)"
                    >{{ $t("document.reject") }}</v-btn
                  >
                </v-col>
              </v-row>
            </v-form>
          </v-card-text>
        </v-card>
      </v-dialog>
      <v-dialog v-model="loading" hide-overlay persistent width="300">
        <v-card color="primary" dark>
          <v-card-text>
            {{ $t("loadingText") }}
            <v-progress-linear
              indeterminate
              color="white"
              class="mb-0"
            ></v-progress-linear>
          </v-card-text>
        </v-card>
      </v-dialog>
    </div>
  </template>
  <script>
  import Cookies from "js-cookie";
  import Swal from "sweetalert2";
  const axios = require("axios").default;
  const moment = require("moment");
  
  export default {
    data: () => ({
      wait: false,
      indeterminate: false,
      base64: null,
      test: false,
      selected_document: null,
      pdfViewDialog: false,
      select_all: false,
      loading: 0,
      sign_type: true,
      docDefinition: [],
      from_date: moment().subtract(1, 'months').format("YYYY-MM-DD"),
      to_date: moment(new Date()).format("YYYY-MM-DD"),
      
      modalDocumentReaction: false,
      search: "",
      reaction: 0,
      dialog: false,
      editMode: null,
      action_types: [
        {
          id: 2,
          name_uz_latin: "Tasdiq",
          name_uz_cyril: "Тасдиқ",
          name_ru: "Утверждение",
        },
        {
          id: 1,
          name_uz_latin: "Rozilik",
          name_uz_cyril: "Розилик",
          name_ru: "Согласование",
        },
        {
          id: 3,
          name_uz_latin: "Bo'lim ichida rozilik",
          name_uz_cyril: "Бўлим ичида розилик",
          name_ru: "Согласование внутри подразделение",
        },
        {
          id: 6,
          name_uz_latin: "Hujjat yaratuvchisi",
          name_uz_cyril: "Ҳужжат яратувчиси",
          name_ru: "Создатель документа",
        },
      ],
      cancelImageUrl:
        "data:image/png;base64,",
      imzoUrl:
        "data:image/png;base64,",
      imageUrl:
        "data:image/png;base64,",
      items: [],
      departments: [],
      roles: [],
      form: {},
      dialogHeaderText: "",
      documentComment: "",
    }),
    computed: {
      screenHeight() {
        return window.innerHeight - 120;
      },
      screenWidth() {
        return window.innerWidth;
      },
      headers() {
        return [
          {
            text: "#",
            value: "id",
            width: 30,
          },
          {
            text: this.$t("name"),
            value: "name",
          },
          {
            text: this.$t("actions"),
            value: "actions",
            width: 50,
            align: "center",
          },
        ];
      },
      language() {
        return this.$i18n.locale == "ru" ? "uz_cyril" : this.$i18n.locale;
      },
    },
    watch: {
      modalDocumentReaction(value) {
        this.AppLoad();
      },
      pdfViewDialog(value) {
        if (!value) {
          this.base64 = null;
          this.selected_document = null;
        }
      },
    },
    methods: {
      makeDocument() {
        let ids = this.items.filter((v) => v.selected).map((v) => v.id);
        axios
          .post(this.$store.state.backend_url + "api/vacation-registry-create", {
            ids: ids,
            locale: this.$i18n.locale,
          })
          .then((res) => {
            this.$router.push("document/" + res.data.pdf_file_name);
          });
      },
      pdfViewInDialog(doc) {
        // this.createDefaultPdf(doc, true);
        this.selected_document = doc;
        this.pdfViewDialog = true;
      },
      selectAll() {
        // this.select_all = !this.select_all;
        this.items = this.items.map((v) => {
          v.selected = this.select_all;
          return v;
        });
      },
      checkSelect() {
        let res = this.items.reduce(
          (accumulator, currentValue) => {
            if (currentValue.selected) accumulator.a++;
            else accumulator.b++;
            return accumulator;
          },
          { a: 0, b: 0 }
        );
        if (res.a != 0 && res.b == 0) this.select_all = true;
        else this.select_all = false;
        this.indeterminate = res.a != 0 && res.b != 0;
      },
      documentSignerData(doc, pageWidth) {
        let data = [{ text: "", margin: [0, 5, 0, 0] }];
        let atLength = this.action_types.length;
        this.action_types.map((action, actinoIdx) => {
          if (
            doc.document_signers &&
            doc.document_signers.find((v) => {
              return action.id == v.action_type_id && action.id != 6
                ? true
                : false;
            })
          ) {
            let tmp = {
              table: {
                widths: [250, "*", "*"],
                body: [
                  [
                    {
                      text: action["name_" + this.$i18n.locale],
                      colSpan: 3,
                      fillColor: "#eef0f7",
                      fontSize: 8,
                    },
                    {},
                    {},
                  ],
                ],
              },
            };
            let cancelImageUrl =
              "data:image/png;base64,";
            let imzoUrl =
              "data:image/png;base64,";
            let imageUrl =
              "data:image/png;base64,";
  
            doc.document_signers
              .filter((f) => f.action_type_id == action.id)
              .map((signer) => {
                let actionColumn = [];
                let updatedAt =
                  signer.updated_at &&
                  signer.updated_at.substr(0, 10) +
                    " " +
                    signer.updated_at.substr(11, 5);
                if (signer.status == 1 && signer.sign_type == 1) {
                  actionColumn = [
                    {
                      image: imzoUrl,
                      width: 20,
                      height: 20,
                      alignment: "center",
                    },
                    {
                      text: updatedAt,
                      alignment: "center",
                      margin: [0, 5, 0, 0],
                      fontSize: 8,
                    },
                  ];
                } else if (signer.status == 1) {
                  actionColumn = [
                    {
                      image: imageUrl,
                      width: 20,
                      height: 20,
                      alignment: "center",
                    },
                    {
                      text: updatedAt,
                      alignment: "center",
                      margin: [0, 5, 0, 0],
                      fontSize: 8,
                    },
                  ];
                } else if (signer.status == 2) {
                  actionColumn = [
                    {
                      image: cancelImageUrl,
                      width: 20,
                      height: 20,
                      alignment: "center",
                    },
                    {
                      text: updatedAt,
                      alignment: "center",
                      margin: [0, 5, 0, 0],
                      fontSize: 8,
                    },
                  ];
                }
                let employeeFIO = "";
  
                if (signer.signer_employee_id) {
                  employeeFIO =
                    signer.signer_employee &&
                    signer.signer_employee["firstname_" + this.language].substr(
                      0,
                      1
                    ) +
                      "." +
                      signer.signer_employee[
                        "middlename_" + this.language
                      ].substr(0, 1) +
                      ". " +
                      signer.signer_employee["lastname_" + this.language];
                } else if (signer.employee_staffs) {
                  employeeFIO =
                    signer.employee_staffs.employee &&
                    signer.employee_staffs.employee[
                      "firstname_" + this.language
                    ].substr(0, 1) +
                      "." +
                      signer.employee_staffs.employee[
                        "middlename_" + this.language
                      ].substr(0, 1) +
                      ". " +
                      signer.employee_staffs.employee[
                        "lastname_" + this.language
                      ];
                } else {
                  employeeFIO = this.$t("document.vacancy");
                }
                tmp.table.body.push([
                  {
                    text:
                      signer.staffs[0].position["name_" + this.$i18n.locale] +
                      "\n" +
                      signer.staffs[0].department["name_" + this.$i18n.locale],
                    fontSize: 10,
                    verticalAlignment: "center",
                    fontSize: 8,
                  },
                  actionColumn,
                  {
                    text: employeeFIO,
                    fontSize: 8,
                  },
                ]);
              });
            data.push(tmp);
          }
        });
        data.push({
          text: [
            {
              text: this.$t("document.creator") + ": ",
              bold: true,
              fontSize: 8,
            },
            doc.employee
              ? doc.employee["lastname_" + this.language] +
                " " +
                doc.employee["firstname_" + this.language].substr(0, 1) +
                "." +
                doc.employee["middlename_" + this.language].substr(0, 1)
              : " ",
            " (",
            this.$t("employee.tabel"),
            " - ",
            doc.employee.tabel,
            ")",
          ],
          margin: [0, 5, 0, 0],
          fontSize: 8,
        });
        return data;
      },
      getFromDepartament(doc) {
        this.loading = this.loading + 1;
        axios
          .post(this.$store.state.backend_url + "api/employees/get-employee", {
            access_type: 100,
            tabel: doc.employee.tabel,
            language: this.$i18n.locale == "ru" ? "uz_cyril" : this.$i18n.locale,
          })
          .then((res) => {
            this.items.map((v) => {
              if (v.id == doc.id) {
                v.from_department = res.data.parents.main_department;
              }
              if (!v.base64) {
                v.newBase64 = true;
              } else v.newBase64 = false;
              return v;
            });
            this.loading = this.loading - 1;
          })
          .catch((err) => {
            console.error(err);
          });
      },
      getList() {
        let locale = this.$i18n.locale;
        this.loading = this.loading + 1;
        axios
          .post(this.$store.state.backend_url + "api/report/lsp-report", {
            fromDate: this.from_date,
            toDate: this.to_date
          })
          .then((response) => {
            this.items = response.data.map((v) => {
              // this.getFromDepartament(v);
              v.selected = false;
              v.document_details = v.document_details.map((dd) => {
                dd.document_detail_contents =
                  dd.document_detail_contents.map((ddav) => {
                    return ddav;
                  });
                return dd;
              });
              return v;
            });
            this.loading = this.loading - 1;
          })
          .catch((error) => {
            console.log(error);
            this.loading = this.loading - 1;
          });
      },
      getDocument(doc) {
        axios
          .post(this.$store.state.backend_url + "api/documents/show-document", {
            id: doc.id,
            language: this.$i18n.locale,
          })
          .then((res) => {
            doc = res.data;
            doc.document_signers.sort((a, b) => {
              return (
                a.staffs[0].department.department_code -
                b.staffs[0].department.department_code
              );
            });
            doc.document_signers.sort((a, b) => {
              return a.sequence - b.sequence;
            });
  
            this.items = this.items.map((v) => {
              if (v.id == doc.id) return doc;
              else return v;
            });
            this.refreshDocument(doc);
            this.items = this.items.filter((v) => v.id != doc.id);
          })
          .catch((err) => {
            console.log(err);
          });
      },
      newItem() {
        this.dialogHeaderText = this.$t("company.newOrganization");
        this.form = {
          id: Date.now(),
          name: "",
          email: "",
          phone: "",
        };
        this.dialog = true;
        this.editMode = false;
        if (this.$refs.dialogForm) this.$refs.dialogForm.reset();
      },
      editItem(item) {
        this.dialogHeaderText = this.$t("company.updateCompany");
        this.formIndex = this.items.indexOf(item);
        this.form = Object.assign({}, item);
        this.dialog = true;
        this.editMode = true;
        if (this.$refs.dialogForm) this.$refs.dialogForm.resetValidation();
      },
      save() {
        if (this.$refs.dialogForm.validate())
          axios
            .post(
              this.$store.state.backend_url + "api/companies/update",
              this.form
            )
            .then((res) => {
              this.getList();
              this.dialog = false;
              const Toast = Swal.mixin({
                toast: true,
                position: "top-end",
                showConfirmButton: false,
                timer: 3000,
                timerProgressBar: true,
                onOpen: (toast) => {
                  toast.addEventListener("mouseenter", Swal.stopTimer);
                  toast.addEventListener("mouseleave", Swal.resumeTimer);
                },
              });
  
              Toast.fire({
                icon: "success",
                title: $t("create_update_operation"),
              });
            })
            .catch((err) => {
              console.log(err);
            });
      }, //companies
      deleteItem(item) {
        const index = this.items.indexOf(item);
        Swal.fire({
          title: this.$t("swal_title"),
          text: this.$t("swal_text"),
          icon: "warning",
          showCancelButton: true,
          confirmButtonColor: "#3085d6",
          cancelButtonColor: "#d33",
          confirmButtonText: this.$t("swal_delete"),
        }).then((result) => {
          if (result.value) {
            this.loading = true;
            axios
              .get(
                this.$store.state.backend_url +
                  "api/vacation-registry-delete/" +
                  item.id
              )
              .then((res) => {
                this.loading = false;
                this.items = this.items.filter((v) => v.id != item.id);
                // this.$router.push("document/" + res.data.pdf_file_name);
              });
          }
        });
      },
      //------------- E-imzo ---------------
      AppLoad() {
        EIMZOClient.API_KEYS = [
          "edo.uzautomotors.com",
          "79DC56F42765A0017C31309DB9600EA924684ED023A8079460454768331626AB94CFFF8FC2D4007976D4A6C56F11D56DFA962276DC54AE8C0F39E8A8EBDFA10B",
        ];
  
        let EIMZO_MAJOR = this.EIMZO_MAJOR;
        let EIMZO_MINOR = this.EIMZO_MINOR;
        let uiLoadKeys = this.uiLoadKeys;
        EIMZOClient.checkVersion(
          function (major, minor) {
            var newVersion = EIMZO_MAJOR * 100 + EIMZO_MINOR;
            var installedVersion = parseInt(major) * 100 + parseInt(minor);
            if (installedVersion < newVersion) {
            } else {
              EIMZOClient.installApiKeys(
                function () {
                  uiLoadKeys();
                },
                function (e, r) {}
              );
            }
          },
          function (e, r) {}
        );
      },
      uiLoadKeys() {
        this.uiClearCombo();
        let uiCreateItem = this.uiCreateItem;
        let eimzo_password = this.$store.getters.getUser().eimzo_password;
        let reactionEimzo = this.reactionEimzo;
        EIMZOClient.listAllUserKeys(
          function (o, i) {
            var itemId = "itm-" + o.serialNumber + "-" + i;
            return itemId;
          },
          function (itemId, v) {
            return uiCreateItem(itemId, v);
          },
          function (items, firstId) {
            var combo = document.testform.key;
            var option = document.createElement("option");
            option.text = "select";
            combo.add(option);
            combo.append(<option value="">Select</option>);
            for (var itm in items) {
              var vo = items[itm].getAttribute("vo");
              if (vo.includes(eimzo_password)) {
                items[itm].setAttribute("selected", "true");
                combo.append(items[itm]);
              }
            }
          },
          function (e, r) {}
        );
      },
      cbChanged(c) {
        Cookies.remove("eimzo_key");
      },
      uiClearCombo() {
        var combo = document.testform.key;
        combo.length = 0;
      },
      reactionEimzo() {
        this.reaction_eimzo = true;
      },
      uiCreateItem(itmkey, vo) {
        var now = new Date();
        vo.expired = dates.compare(now, vo.validTo) > 0;
        var itm = document.createElement("option");
        itm.value = itmkey;
        itm.text = vo.CN;
        if (!vo.expired) {
        } else {
          itm.style.color = "gray";
          itm.text = itm.text + " (срок истек)";
        }
        itm.setAttribute("vo", JSON.stringify(vo));
        itm.setAttribute("id", itmkey);
        return itm;
      },
      getEimzoSigners() {
        let setPkcs7Info = this.setPkcs7Info;
        let base64 = doc.base64;
        if (base64 != null)
          CAPIWS.callFunction(
            {
              plugin: "pkcs7",
              name: "get_pkcs7_attached_info",
              arguments: [
                //Ранее созданный документ PKCS#7/CMS в кодировке BASE64
                // doc.base64,
                base64,
                //Идентификатор хранилища доверенных сертификатов (полученный из фукнции других плагинов), если требуется верификация сертификатов, иначе может быть пустым
                "",
              ],
            },
            function (event, data) {
              setPkcs7Info(data.pkcs7Info);
            },
            function (error) {
              // window.alert(error);
            }
          );
        else alert("Ushbu dokumentga hali imzo chekilmagan.");
      },
      setPkcs7Info(pkcs7Info) {
        this.pkcs7Info = pkcs7Info;
        this.pkcs7InfoDialog = true;
      },
      setBase64(doc, base64) {
        let item = this.items.find((v) => (v.id = doc.id));
        let idx = this.items.indexOf(item);
        doc.base64 = base64;
        this.items[idx] = doc;
        axios
          .post(this.$store.state.backend_url + "api/documents/set-base64", {
            document_id: doc.id,
            base64: base64,
          })
          .then((res) => {});
      },
      documentReaction(doc, reac, signer_serial_number) {
        axios
          .post(this.$store.state.backend_url + "api/document-signers/reaction", {
            document_id: doc.id,
            description: this.documentComment,
            status: reac,
            sign_type: this.sign_type ? 1 : 0,
            staff_id: doc.reaction_staff_id,
            signer_serial_number: signer_serial_number,
          })
          .then((res) => {
            this.getDocument(doc);
            this.modalDocumentReaction = false;
  
            this.pdfViewDialog = false;
            this.selected_document = null;
          })
          .catch((err) => {
            console.log(err);
          });
      },
      sign(doc, reac) {
        var itm = document.testform.key.value;
        var id = document.getElementById(itm);
        var vo = JSON.parse(id.getAttribute("vo"));
        let setBase64 = this.setBase64;
        let documentReaction = this.documentReaction;
        let key = Cookies.get("eimzo_key");
        if (doc.newBase64)
          EIMZOClient.createPkcs7(
            key,
            doc.base64,
            null,
            function (pkcs7) {
              setBase64(doc, pkcs7);
              documentReaction(doc, reac, vo.serialNumber);
            },
            function (e, r) {}
          );
        else
          CAPIWS.callFunction(
            {
              plugin: "pkcs7",
              name: "append_pkcs7_attached",
              arguments: [doc.base64, key],
            },
            function (event, data) {
              if (data["success"]) {
                setBase64(doc, data["pkcs7_64"]);
                documentReaction(doc, reac, vo.serialNumber);
              }
            },
            function (e, r) {}
          );
      },
      verify(doc, reac) {
        if (this.sign_type) {
          this.sign(doc, reac);
        } else {
          this.documentReaction(doc, reac, null);
        }
      },
      actionSelectedDocuments(action) {
        if (this.sign_type && !Cookies.get("eimzo_key")) {
          let itm = document.testform.key.value;
          let id = document.getElementById(itm);
          let vo = JSON.parse(id.getAttribute("vo"));
          let items = this.items;
          let verify = this.verify;
          let reaction = this.reaction;
          let selected_document = this.selected_document;
          EIMZOClient.loadKey(
            vo,
            function (id) {
              CAPIWS.callFunction(
                {
                  plugin: "pfx",
                  name: "verify_password",
                  arguments: [
                    //Идентификатор ключа
                    id,
                  ],
                },
                function (event, data) {
                  if (data.success) {
                    Cookies.set("eimzo_key", id, { expires: 1 / 4 });
                    if (selected_document) {
                      verify(selected_document, reaction);
                    } else
                      items
                        .filter((v) => v.selected)
                        .forEach((doc) => {
                          verify(doc, reaction);
                        });
                  } else
                    Swal.fire({
                      position: "top-end",
                      icon: "error",
                      title: "Wrong E-IMZO password.",
                      showConfirmButton: false,
                      timer: 1500,
                    });
                },
                function (error) {
                  Swal.fire({
                    position: "top-end",
                    icon: "error",
                    title: "Wrong E-IMZO password.",
                    showConfirmButton: false,
                    timer: 1500,
                  });
                }
              );
            },
            function (e, r) {}
          );
        } else {
          if (this.selected_document) {
            this.verify(this.selected_document, this.reaction);
          } else
            this.items
              .filter((v) => v.selected)
              .forEach((doc) => {
                // this.createDefaultPdf(doc, false);
                // this.selected_document = doc;
                // this.pdfViewDialog = true;
                setTimeout(() => {
                  this.verify(doc, this.reaction);
                }, 1000);
              });
        }
      },
      //------------- /E-imzo --------------
    },
    mounted() {
      this.getList();
    },
  };
  </script>
  