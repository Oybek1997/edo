  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("notification.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('notification-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('notification-update')"
                class="px-0"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('notification-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="3" class="pa-0 mt-2 mb-3">
                <v-switch
                  hide-details
                  class="ma-0 pa-0"
              ...
              <v-col cols="12" class="pa-0 mb-3 textField">
                <label class="labelTitle" for>{{ $t('notification.content') }}</label>
                <!-- <v-textarea
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("notification.content"), value: "content" },
        { text: this.$t("notification.is_active"), value: "is_active" },
        {
          text: this.$t("actions"),
          value: "actions",
          width: 120,
  ...
  methods: {
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/notifications", {
    ...
    newItem() {
      this.dialog = true;
      if (this.$store.getters.checkPermission("notification-create")) {
        this.dialogHeaderText = this.$t("notification.create");
        this.form = {
          id: Date.now(),
          content: "",
          is_active: true,
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("notification-update")) {
        this.dialogHeaderText = this.$t("notification.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/notifications/update",
            this.form
          )
          .then((res) => {
            this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("notification-delete")) {
