  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("coefficient.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('coefficient-create')"
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("coefficient.code") }}</label>
                <v-text-field
                  v-model="form.code"
              ...
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("coefficient.description") }}</label>
                <v-text-field
              ...
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("coefficient.order_date") }}</label>
                <v-menu
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("coefficient.code"),
          value: "code",
        },
        {
          text: this.$t("coefficient.description"),
          value: "description",
        },
        {
          text: this.$t("coefficient.order_date"),
          value: "order_date",
        },
        {
          text: this.$t("coefficient.order_number"),
          value: "order_number",
        },
        {
          text: this.$t("actions"),
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("coefficient-update") ||
          this.$store.getters.checkPermission("coefficient-delete")
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/coefficients", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("coefficient-create")) {
        this.dialogHeaderText = this.$t("coefficient.create");
        this.form = {
          id: Date.now(),
          code: "",
          percent: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("coefficient-update")) {
        this.dialogHeaderText = this.$t("coefficient.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/coefficients/update",
            this.form
          )
          .then((res) => {
            this.getList();
