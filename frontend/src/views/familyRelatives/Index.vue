  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("familyRelatives.index") }}</span>
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save">
            <v-row class="mx-0 dialogForm">
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("familyRelatives.status") }}</label>
                <v-autocomplete
                  clearable
                  v-model="form.status"
                  :items="isActives"
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t("name"),
          value: "name_" + this.$i18n.locale,
          width: 250,
        },
        { text: this.$t("Status"), value: "status", width: 250 },
        {
          text: this.$t("actions"),
          value: "actions",
          width: 50,
  ...
  methods: {
    ...
    newItem() {
      ...
      if (this.$store.getters.checkPermission("family-relative-create")) {
        this.dialogHeaderText = this.$t("familyRelatives.index");
        this.form = {
          id: Date.now(),
          name_ru: "",
          name_uz_cyril: "",
    ...
    save() {
      axios
        .post(
          this.$store.state.backend_url + "api/family-relative/update",
          this.form
        )
        .then((res) => {
          this.getList();
