<template>
  <div class="fullHeight">
     <v-card class="pa-0 heightFull" elevation="0">
      ...
      <v-card-title>
         {{   'Всего входящих документов ('+params[param]+')' 
            }}
      </v-card-title>
      <v-card-text>
        <v-data-table
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/document-report-my-item", {
        ...
        .then((response) => {
          ...
          this.okditem = response.data[0];
          // this.dep = response.data[0][0];
          this.param = response.data[1];
