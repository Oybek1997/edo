  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("tariffScale.index") }}</span>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("tariffScale.category") }}</label>
                <v-text-field
              ...
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("tariffScale.salary") }}</label>
                <v-text-field
                  v-model="form.salary"
                  type="number"
                  step="0.01"
              ...
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("tariffScale.order_date") }}</label>
                <v-menu
              ...
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("tariffScale.order_number") }}</label>
                <v-text-field
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("tariffScale.category"), value: "category" },
        { text: this.$t("tariffScale.salary"), value: "salary" },
        { text: this.$t("tariffScale.order_date"), value: "order_date" },
        { text: this.$t("tariffScale.order_number"), value: "order_number" },
        {
          text: this.$t("actions"),
          value: "actions",
          width: 50,
        ...
        (v) =>
          (v.value != "actions" ||
            this.$store.getters.checkPermission("tariff_scale-update") ||
            this.$store.getters.checkPermission("tariff_scale-delete")) &&
          (v.value != "salary" ||
            this.$store.getters.checkPermission("tariff_scale-index_details"))
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/tariff-scales", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("tariff_scale-create")) {
        this.dialogHeaderText = this.$t("tariffScale.create");
        this.form = {
          id: Date.now(),
          category: "",
          salary: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("tariff_scale-update")) {
        this.dialogHeaderText = this.$t("tariffScale.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/tariff-scales/update",
            this.form
          )
          .then((res) => {
            this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("tariff_scale-delete")) {
