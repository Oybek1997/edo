  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("hr_major.index") }}</span>
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('hr-major-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('hr-major-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("hr-major-update") ||
          this.$store.getters.checkPermission("hr-major-delete")
      );
    },
  },
  methods: {
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/hr-majors", {
    ...
    newItem() {
      this.dialog = true;
      if (this.$store.getters.checkPermission("hr-major-create")) {
        this.dialogHeaderText = this.$t("hr_major.add");
        this.form = {
          id: Date.now(),
          name_ru: "",
          name_uz_cyril: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("hr-major-update")) {
    ...
    save() {
      axios
        .post(this.$store.state.backend_url + "api/hr-majors/update", this.form)
        .then((res) => {
          this.getList();
          this.dialog = false;
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("hr-major-delete")) {
