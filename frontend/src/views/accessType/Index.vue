  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("accessType.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('access-type-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('access-type-update')"
                color="blue"
                small
                text
                @click="editItem(item)"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('access-type-delete')"
                color="red"
                small
                text
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("access-type-update") ||
          this.$store.getters.checkPermission("access-type-delete")
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/access-types", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("access-type-create")) {
        this.dialogHeaderText = this.$t("accessType.create");
        this.form = {
          id: Date.now(),
          name_uz_latin: "",
          name_uz_cyril: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("access-type-update")) {
        this.dialogHeaderText = this.$t("accessType.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/access-types/update",
            this.form
          )
          .then((res) => {
            this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("access-type-delete")) {
