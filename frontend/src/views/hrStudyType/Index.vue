  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("hr_study_type.index") }}</span>
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('hr-study-type-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('hr-study-type-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("hr-study-type-update") ||
          this.$store.getters.checkPermission("hr-study-type-delete")
      );
    },
  },
  methods: {
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/hr-study-type")
        .then((response) => {
    ...
    newItem() {
      this.dialog = true;
      if (this.$store.getters.checkPermission("hr-study-type-create")) {
        this.dialogHeaderText = this.$t("hr_study_type.add");
        this.form = {
          id: Date.now(),
          name_ru: "",
          name_uz_cyril: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("hr-study-type-update")) {
    ...
    save() {
      axios
        .post(
          this.$store.state.backend_url + "api/hr-study-type/update",
          this.form
        )
        .then((res) => {
          this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("hr-study-type-delete")) {
