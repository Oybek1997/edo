  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("regions.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('region-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn 
                v-if="$store.getters.checkPermission('region-update')"
                class="px-0"
                color="blue"
                style="min-width: 25px" small text @click="editItem(item)">
                <v-icon size="18">mdi-pencil</v-icon>
              </v-btn>
              <v-btn
                v-if="$store.getters.checkPermission('region-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px" small text @click="deleteItem(item)">
    ...
    >
      <v-card class="pa-5">
        <v-card-title class="pa-0" primary-title>
          <span class="dialogTitle">{{ $t("regions.dialogHeaderText") }}</span>
        </v-card-title>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" sm="6" class="px-1 py-0 mb-3">
                <label class="labelTitle" for>{{ $t("regions.country_id") }}</label>
                <v-autocomplete
                  clearable
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("regions.country_id"), value: "country.name_uz_latin" },
  ...
  methods: {
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/regions")
        .then((response) => {
          this.items = response.data.region;
          this.country = response.data.country.map((v) => {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(this.$store.state.backend_url + "api/regions/update", this.form)
          .then((res) => {
            this.getList();
            this.dialog = false;
    ...
    deleteItem(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete(
              this.$store.state.backend_url + "api/regions/delete/" + item.id
            )
            .then((res) => {
