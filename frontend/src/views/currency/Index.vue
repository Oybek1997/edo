  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("currency.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('currency-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('currency-update')"
                class="pl-0 pr-2"
                color="blue"
              ...
              <v-btn
                v-if="$store.getters.checkPermission('currency-delete')"
                class="pl-0 pr-2"
                color="error"
                style="min-width: 25px"
                small
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("currency.code") }}</label>
                <v-text-field
                  clearable
                  v-model="form.code"
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("currency.name") }}</label>
                <v-text-field
                  clearable
                  v-model="form.name"
                  :rules="[(v) => !!v || $t('input.required')]"
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("currency-update") ||
          this.$store.getters.checkPermission("currency-delete")
      );
    },
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/currencies", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("currency-create")) {
        this.dialogHeaderText = this.$t("currency.newCurrency");
        this.form = {
          id: Date.now(),
          code: "",
          name: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("currency-update")) {
        this.dialogHeaderText = this.$t("currency.updateCurrency");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/currencies/update",
            this.form
          )
          .then((res) => {
            this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("currency-delete")) {
