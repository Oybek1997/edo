  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("departmentTypes.index") }}</span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('department_type-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                v-if="$store.getters.checkPermission('department_type-update')"
                class="pl-0 pr-2"
                color="blue"
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("documentTemplates.sequence") }}</label>
                <v-text-field
...
export default {
  data() {
    return {
      ...
      form: {},
      headers: [
        { text: "#", value: "id",class: "blue-grey lighten-5", align: "center", width: 30 },
        { text: this.$t("name"),class: "blue-grey lighten-5", value: "name_" + this.$i18n.locale },
        { text: this.$t("documentTemplates.sequence"), class: "blue-grey lighten-5",value: "sequence" },
        {
          text: this.$t("actions"),
          value: "actions",
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("department_type-update") ||
          this.$store.getters.checkPermission("department_type-delete")
      ),
      dialogHeaderText: "",
      page: 1,
      from: 0,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/department-types", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("department_type-create")) {
        this.dialogHeaderText = this.$t("departmentTypes.create");
        this.form = {
          id: Date.now(),
          name: "",
          email: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("department_type-update")) {
        this.dialogHeaderText = this.$t("departmentTypes.edit");
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/department-types/update",
            this.form
          )
          .then((res) => {
            this.getList();
