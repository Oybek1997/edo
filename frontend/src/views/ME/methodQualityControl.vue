  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t('Method Quality Control') }}</span>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t('Status nomi') }}</label>
                <v-text-field
                  v-model="form.method_quality_control_name"
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("Status nomi"), value: "method_quality_control_name" },
        ...
        v =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("me-index_model") ||
          this.$store.getters.checkPermission("me-index_model")
      );
    }
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/me/method-quality-control", {
    ...
    newItem() {
      if (this.$store.getters.checkPermission("me-index_model")) {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/me/method-quality-control/update",
            ...
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/me/method-quality-control/delete/" +
                  item.id
              )
              .then(res => {
