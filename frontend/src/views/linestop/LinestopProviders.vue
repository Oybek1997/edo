  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t('linestop.provider') }}</span>
              ...
              <v-list class="dropdown-list pa-0">
                ...
                >
                  <v-list-item-title>
                    ...
                    {{ $t('linestop.add_production_provider') }}
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t('linestop.provider_name') }}</label>
                <v-text-field
                  v-model="form.name"
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t('linestop.region') }}</label>
                <v-text-field
                  v-model="form.region"
                  :rules="[v => !!v || $t('input_required')]"
                  hide-details="auto"
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t('linestop.sapcode') }}</label>
                <v-text-field
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t('linestop.adress') }}</label>
                <v-text-field
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
      {
          ...
          {
            text: "linestop.provider_name",
            class: "blue-grey lighten-5",
            value: "name",
          },
          {
            text: "linestop.region",
            class: "blue-grey lighten-5",
            value: "region",
          },
          {
            text: "linestop.sapcode",
            class: "blue-grey lighten-5",
            value: "sap_code",
          },
          {
            text: "linestop.adress",
            class: "blue-grey lighten-5",
            value: "address",
          },
          {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/get/all-providers", {
    ...
    newItem() {
        this.dialogHeaderText = "linestop.add_production_provider";
        this.form = {
          name: "",
        };
        this.dialog = true;
    ...
    save() {
        axios
          .post(
            this.$store.state.backend_url + "api/saveupdate/provider",
            this.form
          )
          .then(res => {
            this.getList();
