  ...
  <div class="fullHeight">
    <v-card class="heightFull" style="border-radius: 10px; border: 1px solid #dce5ef" elevation="0">
      <v-card-title class="px-4 py-3">
        <span>{{ $t('linestop.report_reason') }}</span>
        <v-spacer></v-spacer>
      </v-card-title>
      <v-card-text class="px-0">
        ...
        <v-row>
          <v-col cols="6">
            <h3 class="ml-5" style="text-align: center">
              {{ $t('linestop.ticket_diagramm') }}
            </h3>
            <PieChart
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: this.$t("linestop.reason_name"), value: "reason_title" },
        { text: this.$t("linestop.all_tickets"), value: "ticket_count", align: "right" },
        { text: this.$t("linestop.total_minute"), value: "total_duration_minutes", align: "right" },
      ],
    };
  },
  methods: {
    getList() {
      this.loading = true;
      axios.post(this.$store.state.backend_url + "api/linestops/reasonstatistics", {
      ...
      .then((response) => {
        ...
        this.chartData = {
          labels: this.items.map((item) => item.reason_title),
          datasets: [{
            data: this.items.map((item) => item.ticket_count),
