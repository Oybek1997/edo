  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t('linestop.reason_stops') }}</span>
          ...
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t('linestop.reason_name') }}</label>
                <v-text-field
                  v-model="form.title"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/get/all-reasons", {
    ...
    newItem() {
        this.dialogHeaderText = this.$t("Добавить новую Причина");
        this.form = {
          name: "",
        };
        this.dialog = true;
    ...
    save() {
        axios
          .post(
            this.$store.state.backend_url + "api/saveupdate/reason",
            this.form
          )
          .then(res => {
            this.getList();
