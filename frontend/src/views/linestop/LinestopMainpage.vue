  ...
  <div class="fullHeight">
    ...
    >
      <v-row class="mx-0 mb-0">
        <v-col cols="3" v-for="(maindata, index) in mainpagedata" :key="index">
          <v-card flat tile>
            <v-hover>
              <template v-slot="{ hover }">
                <v-card
                  ...
                  "
                >
                  <v-card-title class="card_title pa-3">
                    {{ maindata.comment }}
                  </v-card-title>
                  <v-divider class="mx-3" color="#DCE5EF"></v-divider>
                  <v-card-title class="card_title-number px-3 pt-0 pb-3 mt-4">
                    {{ maindata.linestop_count }}
                  </v-card-title>
                </v-card>
              </template>
            </v-hover>
        ...
        >
          <v-card-title class="px-4 py-3">
            <span class="headerTitle mb-2">{{
              $t("linestop.tickets_from_autobots")
            }}</span>
          </v-card-title>
          <v-row class="mx-0">
              ...
              >
                ...
                <template v-slot:[`item.plcdata.stopdt`]="{ item }">
                  ...
                  >
                    {{
                      item.plcdata.stopdt
                        ? item.plcdata.stopdt.slice(0, 19)
                        : " "
                    }}
                  </p>
                </template>
                ...
                <template v-slot:[`item.duration`]="{ item }">
                  ...
                  >
                    {{
                      item.duration
                        ? item.duration.slice(0, 5) + $t("linestop.sec")
                        : "-/-"
                    }}
                  </p>
                </template>
      ...
      <v-card class="pa-3" style="border-radius: 1px">
        ...
        <v-card-text class="pa-0 ma-0">
          ...
          >
            <download-excel
              :data="punkt_excel"
              :name="'Тикеты_с_автоботов_' + today + '.xls'"
            >
...
export default {
  ...
  methods: {
    ...
    viewDay({ date }) {
      if (this.$store.getters.checkPermission("linestop-tmpsending")) {
        this.selectedDate = date;
        this.dialog = true;
        this.getList();
      } else {
        console.log("You do not have permission to view this day.");
      }
    },
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/linestop/singleDateTicket", {
    ...
    getMainPagedata() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/mainpage/getdata")
        .then((response) => {
          this.mainpagedata = response.data;
          this.loading = false;
        })
        .catch((error) => {
          console.log(error);
