  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2" v-if="$route.params.menu_item == 'compliance_restr_two'" style=""> 
          {{ 'Jamiyatga ishga qabul qilinayotgan nomzodlarni tekshirish reestri' }}
        </span>
        <span class="headerTitle mb-2" v-else style=""> 
          {{ 'Manfaatlar to\'qnashuvi bo\'yicha reystr' }}
        </span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                ...
                >
                  <v-list-item-title
                    @click="uploadExcel('table', 'data.xlsx')"
                  >
        ...
        <!-- <v-btn class="mx-3" @click="uploadExcel('table', 'data.xlsx')">
      ...
      <v-row class="mx-0">
        <v-col class="ma-0 pa-0" xs="12">
          <v-data-table 
            id="table" 
            class="doc-template_data-table table_thead_four_line"
            dense
            ...
            }" @update:page="updatePage" @update:items-per-page="updatePerPage" @dblclick:row="rowClick">
            <template v-slot:body.prepend>
              <tr class="py-0 my-0 prepend_height">
                <td style="max-width: 50px;" class="py-0 my-0 dense"></td>
            ...
            <template v-slot:item.document_number="{ item }">
              <td style="max-width: 160px;">
                <v-btn style="height: 21px; border-radius: 10px;" class="success" small :to="'/document/' + item.pdf_file_name">{{ item.document_number_reg ?
            ...
            <template v-slot:item.predmet="{ item }">
              <td style="min-width: 100px; max-width: 200px;">
                {{
                  item.compleans_answer && item.compleans_answer[0] ? (item.compleans_answer[0].answer == 1 ? 'Ha' : 'Yo\'q') : ''
                }}
              </td>
            </template>
            ...
            <template v-slot:item.qrcode="{ item }">
              <img :src="item.qrCode.qrCode" ref="excelImage" />
            </template>
...
export default {
  ...
  methods: {
    ...
    uploadExcel(table, name) {
      // Jadvalda chiqqan rasmning manzilini olish
      const imageUrl = this.$refs.excelImage.src;

      // TableToExcel kutubxonasi orqali ma'lumotlarni Excel fayliga joylashtirish
      // const tableToExcel = new TableToExcel();
      TableToExcel.convert(document.getElementById("table"), {
        name, // Excel faylning nomi
        image: {
          url: imageUrl, // Rasmning manzili
          width: "100px", // Rasmning o'lchami (o'zingizning kerakli o'lchamiga o'zgartiring)
          height: "100px",
        },
      });
    },
