  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2"> {{ $t("Savollar") }} </span>
          ...
          >
            ...
            <v-card>
              <v-list class="dropdown-list pa-0">
                <v-list-item
                  v-if="$store.getters.checkPermission('family-relative-create')"
        ...
        <v-col class="ma-0 pa-0" xs="12">
          ...
          >
            ...
            <template v-slot:item.question="{ item }">
              <td style="min-width: 300px; max-width: 800px; white-space: normal;" class="">
                {{ item.question }}
              </td>
            </template>
            ...
            <template v-slot:item.status="{ item }">
              {{
                item.status == 0 ? $t("employee.inactive") : $t("employee.active")
              }}
            </template>
            <template v-slot:item.actions="{ item }">
              <td style="max-width: 80px;">
                <v-btn
                  v-if="$store.getters.checkPermission('family-relative-update')"
                  class="px-1"
                  color="blue"
                ...
                <v-btn
                  v-if="$store.getters.checkPermission('family-relative-delete')"
                  class="px-1"
                  color="error"
                  style="min-width: 25px"
                  small
          ...
          <v-form @keyup.native.enter="save">
            <v-row class="mx-0 dialogForm">
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("Savol") }}</label>
                <v-text-field
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("Savol turi") }}</label>
                <v-text-field
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        (v) =>
          v.value != "actions" ||
          this.$store.getters.checkPermission("family-relative-update") ||
          this.$store.getters.checkPermission("family-relative-delete")
      );
    },
  },
  methods: {
    getList() {
      this.loading = true;
      axios
        .get(this.$store.state.backend_url + "api/complaens/get-all-questions")
        .then((response) => {
    ...
    newItem() {
      this.dialog = true;
      if (this.$store.getters.checkPermission("family-relative-create")) {
        this.dialogHeaderText = this.$t("Savol qo'shish");
        this.form = {
          id: Date.now(),
          question: "",
          question_type: "",
    ...
    editItem(item) {
      if (this.$store.getters.checkPermission("family-relative-update")) {
    ...
    save() {
      axios
        .post(
          this.$store.state.backend_url + "api/complaens/questions/update",
          this.form
        )
        .then((res) => {
          this.getList();
    ...
    deleteItem(item) {
      if (this.$store.getters.checkPermission("family-relative-delete")) {
            ...
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/complaens/questions/delete/" +
                  item.id
              )
              .then((res) => {
                this.getList();
