<template>
  <v-app id="inspire">
      ...
      <template>
        ...
        >
          ...
          >
            ...
            >
              <!-- style="border: 1px solid red;background-color: yellow;" -->
    ...
    <template>
      <div
      :class="isShown?'prog_bar_close':'prog_bar_open'"
      >
        <v-progress-linear value="100" :color="navBarColor" height="3" />
      </div>
      <button @click="toggleShow" :class="isShown?'toggleClass_open':'toggleClass_close'">
        <v-icon
          style="font-size: 35px;" :color="navBarColor" 
          >{{isShown?'mdi-menu-up':'mdi-menu-down'}}</v-icon
        >
      </button>
    </template>
    ...
    <!-- <template>
        <v-navigation-drawer
          v-if="sideBarVisabled"
          v-model="drawer"
          app
          style="width: 240px; background-color: #f6f5f0"
          id="edonew-sidebar"
        >
    ...
    <v-main
      ...
      v-bind:style="
        sideBarVisabled
          ? 'padding: 90px 5px 45px 300px; background-color: #f6f5f0'
          : 'padding: 90px 5px 45px 300px; background-color: #f6f5f0'
      "
    >
        ...
        .then((response) => {
          response.data.forEach((element) => {
            ...
            this.create_document.push({
              id: element.id,
              icon: element.count>1?"mdi-file-document-multiple-outline":"mdi-file-document-outline",
