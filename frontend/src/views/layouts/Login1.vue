              ...
              <v-form @keyup.native.enter="login" name="testform">
                <v-row>
                  <v-col v-show="e_imzo" cols="12" class="my-2 py-0">
                    ...
                    <br />
                    <v-autocomplete v-model="eimzoKey" :items="eimzoKeys" @change="changeEimzoKey" :error="selectError" :error-messages="selectErrorMessage" item-text="search" return-object item-value="serialNumber"
                      outlined append-icon="">
                      ...
                      <template v-slot:item="{ item }">
                        <div style="width:100%;border-bottom:1px solid #EEE; margin-bottom:15px;">
                          <img src="img/pfx.ico" width="20" height="20"></img> № СЕРТИФИКАТА: {{ item.serialNumber }}<br>
                          <b>ИНН:</b> {{ item.UID }}<br>
                          <b>ПИНФЛ:</b> {{ item.PINFL }}<br> <!-- ФИЗИЧЕСКОЕ ЛИЦО -->
                          <b>Ф.И.О.:</b> {{ item.CN }}<br>
                          <div :style="item.expired ? 'color:red;' : 'color:black;'">Срок действия сертификата ({{getDate(item.validFrom)}} - {{getDate(item.validTo)}})</div>
                  ...
                  <v-col v-if="!e_imzo" cols="12" class="mt-2 mb-0 py-0">
                    <v-text-field color="#203d5b" outlined dense type="password" v-model="password"
                  ...
                  <v-col cols="12" class="mt-0 mb-6 py-0">
                    <v-btn block color="#203d5b" dark @click="e_imzo ? verify() : login()">
                      {{ $t("login") }}
