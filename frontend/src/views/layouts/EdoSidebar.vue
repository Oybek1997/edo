    ...
    <div class="ma-0" id="edoSidebar">
      <div style="height: 95%; overflow-y: auto; overflow-x: hidden;">
        <v-card
          ...
          :key="index"
        >
          <v-list-group value="Actions" class="list-group sidebar_min_height">
            <template v-slot:activator style="min-height: 40px;">
              <v-list-item style="min-height: 38px; max-height: 40px;">
                <v-list-item-content class="py-0 list-text" style="min-height: 40px;">
                  <v-list-item-title style="color: #000; font-weight: 600;">{{ $t(item.name) }}</v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
            ...
            >
              <v-list-item-icon class="px-2 mr-0 my-auto list-icons">
                <v-icon :color="menu.color">{{ menu.icon }}</v-icon>
              </v-list-item-icon>
              <v-list-item-content class="py-0 list-text mr-2">
                <v-list-item-title :title="menu.text"> {{ $t(menu.text) }}</v-list-item-title>
              </v-list-item-content>
            </v-list-item>
          </v-list-group>
        </v-card>
...
export default {
  ...
  mounted() {
    setInterval(() => {
      if (localStorage.getItem("selected_module") && localStorage.getItem("selected_module") !='undefined') {
