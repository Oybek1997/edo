<template>
  <v-app style="background-color: #eef0f7" class="overflow-y-auto">
    <v-app-bar app color="#fffeff" class="elevation-0">
      ...
      <v-toolbar-title>
        <v-row :class="drawer ? 'ml-1' : ''">
          <v-col>
            ...
            <v-col cols="12" class="pa-0 caption">
              {{ $t("contacts") }}:
              <span class="font-weight-bold" v-if="$store.state.PHONE_IT">IT: </span>

              {{ $store.state.PHONE_IT }}

              <span class="font-weight-bold" v-if="$store.state.PHONE_PM">HR: </span>
              ...
              <span class="font-weight-bold" v-if="$store.state.PHONE_PM_TASHKENT">{{
                $t("PM_tashkent")
              }}</span>
      ...
      <v-menu offset-y v-if="notification && notification.length_star && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-2" :title="$t('star')">
            <v-badge
              color="#85d5ff"
            ...
            <v-row>
              <v-list-item :to="'/document/' + item.pdf_file_name">
                <v-col cols="9" class="py-0">
                  <v-list-item-title>
                    <strong>{{ item.document_type["name_" + $i18n.locale] }}</strong>
      ...
      <v-menu offset-y v-if="notification && notification.length_info && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1 mr-6" :title="$t('for_info')">
            <v-badge
              color="grey darken-2"
      ...
      <v-menu
        ...
        v-if="
          notification &&
          notification.length_expected &&
          this.$store.getters.checkPermission('anons') &&
          drawerShow
        "
      >
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-2" :title="$t('Anons')">
            <v-badge
              color="#ccc"
              v-if="notification && notification.length_expected"
              :content="notification && notification.length_expected"
            >
              <p
                style="
                  font-size: 27px;
        ...
        <v-card v-if="notification && notification.length_expected">
          <v-list
            class="py-0"
            style="cursor: pointer"
            dense
            v-for="(item, index) in notification.expected"
            :key="index"
          >
            <v-row>
              <v-list-item :to="'/document/' + item.pdf_file_name">
          ...
          <v-list class="py-0" style="cursor: pointer" dense>
            <v-list-item to="/documents/list/expected/0">
              <v-list-item-title>{{ $t("message.all") }}</v-list-item-title>
            </v-list-item>
          </v-list>
        </v-card>
      ...
      <v-menu offset-y v-if="notification && notification.length_prosesing && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-2" :title="$t('pending')">
            <v-badge
      ...
      >
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-2" :title="$t('substantiate')">
            <v-badge
      ...
      <v-menu offset-y v-if="notification && notification.length_executor && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1" :title="$t('executor')">
            <v-badge
              color="pink darken-4"
      ...
      >
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1" :title="$t('resolutions')">
            <v-badge
      ...
      <v-menu offset-y v-if="notification && notification.length_results && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1" :title="$t('resolution_results')">
            <v-badge
              color="light-blue darken-4"
      ...
      <v-menu offset-y v-if="notification && notification.length_watcher && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1" :title="$t('expired')">
            <v-badge
      ...
      <v-menu offset-y v-if="notification && notification.length_canceled && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1" :title="$t('negative')">
            <v-badge
              color="red darken-2"
      ...
      <v-menu offset-y v-if="notification && notification.length_agreement && drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1" :title="$t('negative')">
            <v-badge
            ...
            >
              <v-icon color="light-green darken-4">mdi-file-check-outline</v-icon>
            </v-badge>
      ...
      <v-menu offset-y v-if="drawerShow">
        <template v-slot:activator="{ on }">
          <v-btn text small v-on="on" class="ml-1" :title="$t('document.news_document')">
            <v-badge
              color="green"
      ...
      <v-menu offset-y>
        ...
        <v-list dense>
          <v-list-item
            to="/koreshok/list"
            v-if="$store.state.COMPANY_ID == 1 && user && !user.type && drawerShow"
          >
          ...
          <v-list-item
            ...
            "
          >
            <v-list-item-title>{{ $t("message.profileDialog") }}</v-list-item-title>
          </v-list-item>
          <v-list-item v-if="this.$store.getters.checkPermission('tabel') && drawerShow">
            <v-list-item-title>
              <router-link to="/tabel" style="text-decoration: none; color: inherit">{{
                $t("tabel")
              }}</router-link>
            </v-list-item-title>
          </v-list-item>
                ...
                <v-row>
                  <v-col cols="4">
                    <v-item v-slot="{ toggle }">
                      <v-card class="text-center pa-2" height="80" width="80" @click="toggle" flat to="/ism">
          ...
          <v-list dense subheader class="left-aside">
            <v-list-group :title="$t('message.create')">
              ...
              <template v-for="(item, i) in create_document">
                <v-list-item
                  v-if="item.id == 0 && user && !user.type ? true : item.visible"
                  :key="i"
                  router
                  :to="item.route"
                  class="pl-4"
            ...
            <v-list-group>
              <template v-slot:activator>
                ...
                <v-list-item-content>
                  <v-list-item-title>{{ $t("my_documents") }}</v-list-item-title>
                </v-list-item-content>
              </template>
              <v-list-group
                sub-group
              ...
              >
                ...
                <template>
                  ...
                  >
                    ...
                    <v-list-item-content>
                      <v-list-item-title v-text="$t('uzauto')"></v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                </template>
                <template> </template>
                ...
                >
                  <v-list-item-content class="ml-6">
                    <v-list-item-title v-text="$t('document.all')"></v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
              </v-list-group>
            </v-list-group>
            ...
            <v-list-item router to="/documents/signed" class="pl-4" style>
              <v-list-item-icon class="mr-1">
                <v-icon>mdi-file-document-outline</v-icon>
              </v-list-item-icon>
              <v-list-item-content>
            ...
            >
              ...
              <template>
                <v-list-item
                  v-if="$store.getters.checkPermission('complaens-cancel-documents')"
                  router
                  to="/complaens/cencel-document"
                  class="pl-4"
                  :title="$t('cancel_documents')"
                >
                  <v-list-item-icon class="mr-1" :title="$t('cancel_documents')">
                    <v-icon small>mdi-file-document-outline</v-icon>
                  </v-list-item-icon>

                  <v-list-item-content>
                    <v-list-item-title
                      v-text="$t('cancel_documents')"
                    ></v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
              </template>
            ...
            <v-list-group :title="$t('message.reports')">
              <template v-slot:activator>
                <v-list-item-icon class="mr-1">
                  <v-icon>mdi-file-document-outline</v-icon>
                </v-list-item-icon>
            ...
            <v-list-group :title="$t('staff.hr')" v-if="!!hrLinks.find((v) => v.visible)">
              <template v-slot:activator>
                <v-list-item-icon class="mr-1">
            ...
            <v-list-item router to="/car-purchases/list" class="pl-4" v-if="false">
              <v-list-item-icon class="mr-1">
            ...
            <!--Blank templates-->
            <v-list-group v-if="$store.getters.checkPermission('blank-template-index')">
              <template v-slot:activator>
                <v-list-item-icon class="mr-1">
            ...
            <v-list-group
              :title="$t('archive')"
              v-if="!!this.archiveLinks.find((v) => v.visible)"
            >
              <template v-slot:activator>
                <v-list-item-icon class="mr-1">
              ...
              v-for="(item, index) in [
                {
                  ...
                  target: '',
                  visible:
                    $store.state.COMPANY_ID == 3 && user && user.username == 'test',
                },
                {
                  ...
                  icon: 'mdi-file-outline',
                  target: '',
                  visible: $store.state.COMPANY_ID == 1,
                },
                {
    ...
    <v-main>
      ...
      <v-layout
        style="position: fixed; bottom: 30px; right: 10px; width: 50px; z-index: 1000"
      >
        <v-row class="ma-0 pa-0">
          <v-col cols="12" class="ma-0 pa-0">
            <v-btn v-if="false" fab dark color="success" @click="toTop" class="mb-2">
    ...
    >
      <v-card>
        <v-card-title class="headline grey lighten-2" primary-title>
          {{ $t("message.settings") }}
          <v-spacer></v-spacer>
          <v-btn color="red" dark x-small fab class @click="settingDialog = false">
    ...
    >
      <v-card>
        <v-card-title class="headline grey lighten-2" primary-title>
          {{ $t("message.myCompany") }}
          <v-spacer></v-spacer>
          <v-btn color="red" dark x-small fab class @click="myCompanyDialog = false">
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
        <v-card-text class="my-2"></v-card-text>
    ...
    >
      <v-card style="height: 100%">
        <v-card-title class="headline grey lighten-2" primary-title>
          {{ $t("message.profile") }}
          <v-spacer></v-spacer>
          <v-btn color="red" dark x-small fab class @click="myProfileDialog = false">
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
        ...
        <v-row class="ma-0">
          ...
          <v-col
            ...
            md="12"
            v-if="
              employee.eimzo_name || employee.eimzo_username || employee.eimzo_password
            "
          >
            <v-card>
              <v-card-text class="pa-2 text-center" v-if="employee.eimzo_password">
          ...
          <v-form>
            <v-row>
              ...
              <v-col cols="2" style="min-width: 100px" class="px-0">
                <v-btn class="mt-6" color="success" block @click="successUploadPhoto()"
                  >+</v-btn
                >
              </v-col>
            </v-row>
...
export default {
    ...
    hrLinks() {
      return [
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("vacation-registry"),
        },
        {
          icon: "mdi-clipboard-account-outline",
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("structure-company-index"),
        },
        {
          icon: "mdi-clipboard-account-outline",
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("hr-military-rank-index"),
        },
        {
          icon: "mdi-clipboard-account-outline",
    ...
    adminLinks() {
      return [
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("show_unblocked_user_list"),
        },
        {
          icon: "mdi-folder-open",
    ...
    inventoryLinks() {
      return [
        ...
        },
        {
          ...
          text: this.$t("Blankalar"),
          route: "/inventory/blanks",
          visible:
            this.$store.getters.checkRole("inventory_operator") ||
            this.$store.getters.checkRole("inventory_controller") ||
            this.$store.getters.checkRole("inventory_report"),
    ...
    staffLinks() {
      return [
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("access-department-index"),
        },
        {
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("requirement_type-index"),
        },
        {
          icon: "mdi-webhook",
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("sap-transactions-index"),
        },
        {
          icon: "mdi-clipboard-account-outline",
      ...
      return [
        {
          ...
          visible: this.$store.getters.checkPermission("document_template-index"),
        },
        {
          icon: "mdi-folder-open",
        ...
        },
        {
          ...
          visible: this.$store.getters.checkPermission("purchase_catalog-index"),
        },
        {
          icon: "mdi-checkbox-multiple-marked-outline",
  ...
  mounted() {
    ...
    axios
      .get(
        this.$store.state.backend_url + "api/documents/notification/" + this.$i18n.locale
      )
      .then((res) => {
