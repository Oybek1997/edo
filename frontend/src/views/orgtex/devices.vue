  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t("device.indexes") }}</span>
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("device.model") }}</label>
                <v-text-field
                  v-model="form.model"
                ...
                ></v-text-field>
                <datalist id="models">
                  <option v-for="(item, indModel) in models" :key="indModel">{{ item.model }}</option>
                </datalist>
              </v-col>
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("device.serial_number") }}</label>
                <v-text-field
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("device.inventory_number") }}</label>
                <v-text-field
              ...
              <v-col cols="12" class="pa-0 mb-3">
                <label class="labelTitle" for>{{ $t("device.use_date") }}</label>
                <v-menu
                v-model="menu"
                :close-on-content-click="false"
                :nudge-right="40"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/orgtex/devices", {
    ...
    newItem() {      
      if (this.$store.getters.checkPermission("orgtex-admin")) {
        ...
        this.form = {
          id: Date.now(),
          type: "",
          first_use_date: (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10),
        };
        this.dialog = true;
        this.editMode = false;
    ...
    getModel() {
      axios
        .get(this.$store.state.backend_url + "api/orgtex/models")
        .then(response => {
