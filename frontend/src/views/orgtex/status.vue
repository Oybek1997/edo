      ...
      >
        ...
        <template v-slot:item.employee="{ item }">
          <span v-if="$i18n.locale == 'uz_latin'">
            {{ item.history.employee ? item.history.employee.lastname_uz_latin : ""}}
            {{ item.history.employee ? item.history.employee.firstname_uz_latin : "" }}
          </span>
          <span v-else>
            {{ item.history.employee ? item.history.employee.firstname_uz_cyril : "" }}
            {{ item.history.employee ? item.history.employee.lastname_uz_cyril : "" }}
          </span>
        </template>
        <template v-slot:item.created_at="{ item }">{{ item.history.created_at.slice(0, 10) }}</template>
        <template v-slot:item.first_use_date="{ item }">{{ item.history.first_use_date.slice(0, 10) }}</template>
        <template v-slot:item.status="{ item }">
          <template v-if="item.history && item.history.status == 1">Принял</template>
          <template v-else-if="item.history && item.history.status == 2">Отдал</template>
          <template v-else-if="item.history && item.history.status == 3">Ремонт</template>
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("Tabel"), value: "history.employee.tabel" },
        {
          text: this.$t("user.employee"),
          value: "employee"
        },
        {
          text: this.$t("user.department_code"),
          value: "history.department_code"
        },
        {
          text: this.$t("user.department_id"),
          value: "history.employee_department"
        },
        { text: this.$t("user.position"), value: "history.employee_position" },
        { text: this.$t("device.act_number"), value: "history.act_number" },
        { text: this.$t("device.use_date"), value: "history.first_use_date" },
        { text: this.$t("device.reg_date"), value: "created_at" },
        { text: this.$t("device.user"), value: "history.created_by.username" },
        { text: this.$t("description"), value: "history.description" }
      ];
    }
  },
  methods: {
