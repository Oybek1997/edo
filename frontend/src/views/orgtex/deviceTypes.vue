  ...
  <div class="fullHeight">
    ...
    >
      <v-card-title class="px-4 py-3">
        <span class="headerTitle mb-2">{{ $t('device.types') }}</span>
          ...
          >
            <template v-slot:body.prepend="{ item }">
              <tr class="prepend_height">
                <td></td>
                <td>
                  <v-text-field v-model="filter.type" hide-details dense @keyup.enter="getList"></v-text-field>
                </td>
                <td>
                  <v-autocomplete
                    clearable
                    v-model="filter.device_branch_id"
                    :items="
                      deviceBranches.map(v => ({
                        text: v.name,
                        value: v.id,
                      }))
                    "
        ...
        <v-card-text>
          <v-form @keyup.native.enter="save" ref="dialogForm">
            <v-row class="mx-0 dialogForm">
              <v-col cols="6" class="pl-0 pr-1 mb-3">
                <label class="labelTitle" for>{{ $t('device.type') }}</label>
                <v-text-field
                  v-model="form.type"
              ...
              <v-col cols="6" class="pl-0 pr-1 mb-3">
                <label class="labelTitle" for>{{ $t("device.branch") }}</label>
                <v-autocomplete
                  outlined
                  class="pa-0"
                  clearable
                  v-model="form.device_branch_id"
                  :items="deviceBranches"
                  :item-text="'name'"
                  item-value="id"
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: this.$t("device.type"), value: "type" },
        { text: this.$t("device.branch"), value: "device_branch.name" },
  ...
  methods: {
      ...
      axios
        .get(
          this.$store.state.backend_url +
            "api/orgtex/device-type/get-ref-branch"
        )
        .then(response => {
          this.deviceBranches = response.data;
          this.loading = false;
        })
        .catch(error => {
          console.log(error);
    ...
    getList() {
      this.loading = true;
      axios
        .post(this.$store.state.backend_url + "api/orgtex/device-types", {
    ...
    save() {
      if (this.$refs.dialogForm.validate())
        axios
          .post(
            this.$store.state.backend_url + "api/orgtex/device-types/update",
        ...
        Swal.fire({
          ...
          if (result.value) {
            axios
              .delete(
                this.$store.state.backend_url +
                  "api/orgtex/device-types/delete/" +
                  item.id
              )
              .then(res => {
