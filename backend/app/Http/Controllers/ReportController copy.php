...
class ReportController extends Controller
{
    ...
    public function okdReportFull(Request $request)
    {
        ...
        $mainDep = Department::select('id')->where('parent_id', 1)->get()->pluck('id')->toArray();
    ...
    public function OkdReportItemFull(Request $request)
    {
        ...
        return $doc = $this->templateFull($row, $dep_id, $lang, $from_date, $to_date, $filter, $locale, $parent, $template_id );
    }

    public function templateFull($request, $dep_id, $lang, $from_date, $to_date, $filter,  $locale, $parent, $template_id)
    {
        $one_date = date('Y-m-d', strtotime('+1 days'));
        $three_date = date('Y-m-d', strtotime('+3 days'));
    ...
    public function okdReport(Request $request)
    {
        $from_date = !empty($request['search']['from_date']) ?
            $request['search']['from_date'] :
        ...
        $controlDoc = $request['search']['controlDoc'];
        // if($controlDoc==1):
        ...
        // $departments = [827, 226, 709, 269, 720, 626, 721, 141, 741, 932, 742, 1111, 748, 1142, 750, 662, 758, 840, 931, 5, 1080, 689, 268, 693, 1129, 839, 67, 1104, 72, 132, 547, 612, 1126, 617, 1127, 8, 1128, 679, 3, 2, 4, 216, 11, 557, 14, 824, 204, 18, 171, 179, 181, 891, 183, 199];
        ...
        foreach ($state_ids as $key => $item) :
            ...
            $document_all = DocumentSigner::select('id')
                ...
                ->where('action_type_id', 4)
                ->where('status', '!=', 2)
                ->whereHas('Documents', function ($q) use ($controlDoc, $signer_group_id) {
                    if (in_array($controlDoc, [1, 2, 3, 4]))
                        $q->whereHas('DocumentSigners', function ($qr) use ($signer_group_id) {
                            $qr->whereIn('signer_group_id', $signer_group_id);
                        });
                    $q->whereHas('documentTemplate', function ($query) use ($controlDoc) {
                        if ($controlDoc  == 2)
                            $query->whereIn('id', [71, 305]);
                        if ($controlDoc  == 3)
                            $query->whereIn('id', [70, 190]);
                        if ($controlDoc  == 4)
                            $query->whereIn('id', [214, 357]);
                    });
                })
                ->whereBetween('created_at', [$from_date, $to_date])
                ->whereDoesntHave('Documents', function ($q) {
    ...
    public function OkdReportItem(Request $request)
    {
        $filter = $request->input('filter');
        // $from_date = ($request['route_array'][2] != null) ?

        //     date('Y-m-d', strtotime('-1 month')) :
        //     $request['route_array'][2];

        // $to_date = ($request['route_array'][3] != null) ?
        //     date('Y-m-d') :
        //     $request['route_array'][3];
        $from_date = (!$request['route_array'][2]) ?
        ...
        $controlDoc = $request['route_array'][4];
        $signer_group_id = [21, 22, 23, 24, 143, 161, 241, 274, 275, 278];

        return $doc = $this->template($row, $dep_id, $signer_group_id, $lang, $from_date, $to_date, $filter, $locale, $controlDoc);
    }

    public function template($request, $dep_id, $signer_group_id, $lang, $from_date, $to_date, $filter,  $locale, $controlDoc)
    {
        $one_date = date('Y-m-d', strtotime('+1 days'));
        $three_date = date('Y-m-d', strtotime('+3 days'));
        $mainDep = Department::select('id')->where('parent_id', 1)->get()->pluck('id');
    ...
    public function okdReportTab(Request $request)
    {
        $from_date = (!$request['route_array'][1]) ?
            date('Y-m-d', strtotime('-1 month')) :
            $request['route_array'][1];

        $to_date = (!$request['route_array'][2]) ?
            date('Y-m-d') :
            $request['route_array'][2];

        $controlDoc = $request['route_array'][3];
        $signer_group_id = [21, 22, 23, 24, 143, 161, 241, 274, 275, 278];
    ...
    public function documentReportMy(Request $request)
    {
        ...
        $myDoc = [0 => 'Всего входящих документов'];
