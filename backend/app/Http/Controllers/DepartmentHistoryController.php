...
use App\Http\Models\DepartmentHistory;
...
class DepartmentHistoryController extends Controller
{
    ...
    {
        $departments = DepartmentHistory::with(['managerStaff' => function ($q) {
    ...
    public function indexView(Request $request)
    {
        ...
        $departments = DepartmentHistory::with('company')
            ...
            // ->leftJoin('departments as parent', 'parent.id', '=', 'department_histories.parent_id')
            // // ->whereIn('departments.branch_id', $branches)
            // ->orderBy('department_histories.department_code', 'asc');
        if (isset($filter['department_name'])) {
            $departments->where(function (Builder $query) use ($filter) {
                return $query
                    ->where("department_name", 'like', "%" . $filter['department_name'] . "%");
            });
        }
        ...
        if (isset($filter['department_type_id'])) {
            $departments->where('department_type_id', $filter['department_type_id']);
        }
        ...
        if (isset($filter['functional_department_name'])) {
            if($filter['functional_department_name'] == 0){
                $departments->whereNull('functional_department_id');
            }else{
                $departments->whereHas('functionalDepartment', function($q) use($filter){
        ...
        if (isset($filter['department_code'])) {
            $departments->where('department_code', 'like', '%' . $filter['department_code'] . '%');
        }
        if (isset($filter['branch_id'])) {
            $departments->where('branch_id', $filter['branch_id']);
        }
        return [
            'departments' => $departments->paginate($itemsPerPage == '-1' ? 1000000 : $itemsPerPage, ['*'], 'page name', $page),
            // 'filter' => $filter,
        ];
    }
    public function getDepartmentsToHistory(){
        $monthNumber = date("m");
        $yearNumber = date("Y");
        // return $yearNumber;
        $departments = Department::where('is_active', 1)->get();
        foreach ($departments as $key => $department) {
            $history = DepartmentHistory::where('department_id', $department->id)->where('month', $monthNumber)->where('year', $yearNumber)->first();
            if(!$history){
                $history = new DepartmentHistory();
            }
            $history->department_id = $department->id;
            $history->department_code = $department->department_code;
            $history->department_name = $department->name_uz_latin;
            $history->department_type_id = $department->department_type_id;
            $history->company_id = $department->company_id;
            $history->parent_id = $department->parent_id;
            $history->branch_id = $department->branch_id;
            $history->manager_staff_id = $department->manager_staff_id;
            $history->functional_department_id = $department->id;
            $history->functional_department_code = $department->id;
            $history->functional_parent_id = $department->id;
            $history->month = $monthNumber;
            $history->year = $yearNumber;
    ...
    public function getList(Request $request)
    {
        ...
        if (isset($search)) {
            $department->where(function ($query) use ($search) {
                return $query
                    ->where('departments.department_code', 'like', $search . "%")
                    ...
                    ->orWhere(DB::raw("concat(employees.lastname_uz_latin,' ', employees.firstname_uz_latin)"), 'like', "%" . $search . "%")
                    ->orWhere(DB::raw("concat(employees.lastname_uz_cyril,' ', employees.firstname_uz_cyril)"), 'like', "%" . $search . "%")

                    ->orWhere(DB::raw("concat(employees.firstname_uz_latin, ' ', employees.lastname_uz_latin)"), 'like', "%" . $search . "%")
                    ->orWhere(DB::raw("concat(employees.firstname_uz_cyril, ' ', employees.lastname_uz_cyril)"), 'like', "%" . $search . "%")
